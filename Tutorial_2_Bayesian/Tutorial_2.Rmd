...
M1_data <- ptb_data %>% select(-c(bl,tr,trial,period, period_9)) # 12840 obs. of 14 variables
head(M1_data)
summary(M1_data)
```
...
priors_M1 <- c(
  ...
  set_prior("skew_normal(-1,1,-2)", class = "b", coef = "d9"), 
...
model_M1 <-
   brm(data = M1_data,
       ...
       event | trials(1) ~ 0 + d6 + d7 + d8 + d9 + d10 + d11 + d12 + d13 + d14 + d15 + condition + trial_c +
                          (0 + d6 + d7 + d8 + d9 + d10 + d11 + d12 + d13 + d14 + d15 + condition + trial_c | pid),
       prior = priors_M1,
...
Model_M1 took about 96 minutes on a MacBook Pro (Sonoma 14.6.1 OS, 18GB Memory, M3 Pro Chip).
...
priors_M2 <- c(
  set_prior("skew_normal(-0.2,0.71,-2.2)", class = "b"),       # weakly informative prior for beta parameters when using cloglog link
  set_prior("skew_normal(-1,1,-2)", class = "Intercept"),      # weakly informative prior for Intercept when using cloglog link
  set_prior("normal(0, 1)", class = "sd"),                     # prior for standard deviation of random effects
  set_prior("lkj(2)", class = "cor")                           # prior for correlations between random effects
)
```
...
model_M2 <-
   brm(data = M2_data,
       family = binomial(link="cloglog"),
       event | trials(1) ~ 0 + Intercept + period_9 + I(period_9^2) + I(period_9^3) + condition + trial_c +
                          (1 + period_9 + I(period_9^2) + I(period_9^3) + condition + trial_c | pid),
       prior = priors_M2,
       chains = 4, cores = 4, iter = 3000, warmup = 1000,
       control = list(adapt_delta = 0.999, step_size = 0.04, max_treedepth = 12),
       seed = 12, init = "0",
...
# Plot effects for selected model
