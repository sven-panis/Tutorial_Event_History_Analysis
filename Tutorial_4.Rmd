...
data <- read_csv("Tutorial_4_planning/data/tidyppp_Exp1_n6.csv") |>
  ...
  mutate(
    prime = if_else(con_pr == 1, "con", "inc"),
    prime = factor(prime,
      levels = c("con", "inc")
    ),
...
tidy_fixed <- post |>
  ...
  mutate(
    key = factor(name, levels = unique(name)),
...
```{r plot-bi-hazards}
p4.2 <- ggplot(tidy_haz, aes(
  x = bin, y = hazard,
  fill = prime
)) +
  stat_halfeye(alpha = 0.7) +
  labs(
    title = "hazard estimates",
    x = "time bin", y = "hazard"
  ) +
  scale_fill_brewer(palette = "Dark2") +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25))
...
cor_mat_tidy <- as_tibble(cor, rownames = "term") |> ##
  ...
  pivot_longer(-term,
    ...
    names_pattern = "([a-zA-Z_\\d?]+)_([a-zA-Z:\\d?]+)"
  ) |> ## pivot longer and use names_pattern to select the parts of the column names to separate on. the above is regex for each bit between the separator ("-"). e.g., ([a-zA-Z]+\\d+). this looks for letters and digits before the first "_". This (\\d?) looks for the possibility of digits.
...
d1 <- add_random(subj = subj_n, rep = rep_n) |>
  ...
  add_contrast("condition", "treatment",
    add_cols = TRUE,
    colnames = c("cond")
  ) |>
  ...
  mutate(
    con = if_else(condition == "cond1", 1, 0),
    inc = if_else(condition == "cond2", 1, 0)
  ) |>
  mutate(
    bin1 = if_else(bin == 1, 1, 0),
  ...
  # add random effects
  add_ranef("subj",
    u1c = u1c_sd, u2c = u2c_sd, u3c = u3c_sd, u4c = u4c_sd,
...
p5.1 <- ggplot(d1_group, aes(x = bin, y = cloglog, colour = condition)) +
  ...
  scale_colour_brewer(palette = "Dark2") +
  labs(
    title = "cloglog - simulation d1 (N=10)",
    x = "time bins"
  )
p5.1
```
...
p5.2 <- ggplot(d1_group, aes(x = bin, y = event, colour = condition)) +
  ...
  scale_colour_brewer(palette = "Dark2") +
  labs(
    title = "event prob - simulation d1 (N=10)",
    x = "time bins"
  )
p5.2
```
...
d1_out_pid <- d1_out |>
  ...
  mutate(
    prime = if_else(condition == "cond1", "con", "inc"),
    prime = factor(prime)
  )
d1_out_pid
...
sx1_exp <- sx1 |>
  ...
  summarise(
    exp_mean_outcome = mean(mean_outcome, na.rm = TRUE),
...
sx1_sim <- sx1_exp |>
  ...
  summarise(
    sim_mean_outcome = mean(exp_mean_outcome, na.rm = TRUE),
...
p9.1 <- ggplot(
  sx1_sim,
  aes(
    x = bin, y = sim_mean_outcome,
    colour = condition
  )
) +
  geom_jitter(
    data = sx1_exp, aes(y = exp_mean_outcome),
...
sx1_diff_exp <- sx1 |>
  pivot_wider(
    id_cols = c(exp, subj_n, rep_n, b6c, subj, bin),
    names_from = "condition",
  ...
  summarise(
    exp_mean_diff = mean(diff, na.rm = TRUE),
...
sx1_diff_sim <- sx1_diff_exp |>
  ...
  summarise(
    mean_diff = mean(exp_mean_diff, na.rm = TRUE),
...
p10.1 <- ggplot(sx1_diff_exp, aes(
  x = bin, y = exp_mean_diff,
  colour = bin, fill = bin
)) +
...
p11.1 <- ggplot(
  ...
  geom_jitter(
    data = sx2_exp, aes(y = exp_mean_outcome),
    alpha = 0.5, width = 0.1, height = 0
  ) +
  geom_line(aes(group = condition)) +
  geom_point(size = 2, colour = "black") +
