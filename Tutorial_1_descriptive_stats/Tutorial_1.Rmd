...
lifetable_neutral_s6 <- data_final[[7]][[6]] %>% 
...
riskset <- lifetable_neutral_s6 %>% select(risk_set) %>% slice(1) %>% pull()
...
# add new row
lifetable_neutral_s6_complete <- lifetable_neutral_s6 %>%
                                 add_row(!!! row0) %>%
                                 arrange(bin)
lifetable_neutral_s6_complete
```

```{r save-single-lifetable}
write_csv(lifetable_neutral_s6_complete, file="Tutorial_1_descriptive_stats/tables/lifetable_neutral_s6.csv")
```
