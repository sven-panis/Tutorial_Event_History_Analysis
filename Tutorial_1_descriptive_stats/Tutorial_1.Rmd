...
This file wrangles raw data from Experiment 1 of Panis and Schmidt (2016), and illustrates how to use functional programming to set up life tables, and plot the discrete-time hazard, survivor, and conditional accuracy functions, when there is one independent variable.
...
# change variable names (to pid, trial, condition, rt, and acc)
colnames(data_wr) <- c("pid","bl","tr","condition","resp","acc","rt","trial") 
# pid to indicate column with person IDs
# trial must be a unique trial number per subject
# condition must be a factor indicating the experimental levels of the single independent variable (1,2,...)
# rt to indicate column with response times
# acc to indicate column with accuracies

# create factor for condition (prime type)
data_wr <- data_wr %>% mutate(condition = condition + 1,
                              condition = factor(condition, levels = c(1,2,3), labels = c("neutral","congruent","incongruent")))
summary(data_wr)
```
...
# function extract_median is used to extract the S(t).50 quantiles (estimated median RTs)
...
plot_eha <- function(df,subj,haz_yaxis){ # set the upper limit of the y-axis for the hazard functions to haz_yaxis == 1 when plotting the data of individual subjects
  library(patchwork)
...
p2 <-df %>%
  ...
  geom_path(aes(x=period, y=survival, color=factor(condition, levels =c(1,2,3),labels=c("neutral","congruent","incongruent"))),
            data = data_medians, 
...
# Extract info, save plot for each subject, create person-trial-bin oriented data file for hazard modeling, and a data file for ca(t) modeling.
...
pluck(data_final, 7, 1) %>% filter(condition=="neutral")
...
# Optional: Create single plot for data aggregated across subjects
