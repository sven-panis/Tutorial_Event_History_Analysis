...
ptb <- function(df) {
  df |>
    uncount(weights = drt) |> # create drt rows per trial
    group_by(trial) |>
...
setup_lt <- function(ptb) {
  ptb |>
    ...
    # replace NA with 0
    mutate(
      event0 = ifelse(is.na(event0), 0, event0),
    ...
    mutate(
      se_haz = sqrt((hazard * (1 - hazard)) / risk_set) |> round(digits = 4),
    ...
    mutate(
      survival = (cumprod(1 - hazard)) |> round(digits = 4),
...
join_lt_ca <- function(df1, df2) {
  df1 |>
    left_join(df2, join_by(condition, period)) |>
    select(
      condition, period, event1, risk_set, hazard, se_haz, survival, se_surv,
...
plot_eha <- function(df, subj, haz_yaxis = 1, first_bin_shown = 1, aggregated_data = F, Nsubj = 6) {
  ...
  p1 <- df |> ggplot(aes(x = period, color = condition, group = condition)) +
    ...
    scale_x_continuous(
      breaks = c(first_bin_shown:(cutoff / binsize)),
...
join_lt_ca_2IV <- function(df1, df2) {
  df1 |>
...
plot_eha_2IV <- function(df, subj, haz_yaxis = 1, first_bin_shown = 1, aggregated_data = F, Nsubj = 6) {
  ...
  p1 <- df |>
    mutate(
      mask = condition1, # Create correctly named factor (here: mask)
      prime = condition2
    ) |> # Create correctly named factor (here: prime)
  ...
  p2 <- df |>
    mutate(
      mask = condition1, # Create correctly named factor
      prime = condition2
    ) |> # Create correctly named factor
    ggplot(aes(x = period, color = prime, group = prime)) + # Apply correctly named factor
