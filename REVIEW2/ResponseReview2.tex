...
We appreciate the time taken by you to provide further feedback, as your
comments have enabled us to improve the manuscript. We document our
...
Response: We appreciate that making this paper as accessible as possible
is a key priority, given that one barrier to more use of event history
analysis is the relative complexity of the approach compared to more
conventional analyses. As such, we have taken a lot of time and effort
to first consider what kind of accessibility we are aiming to achieve,
as well as how we might achieve it. Our revised approach to making the
paper accessible has several strands. First, consistent with our initial
aims and instincts, we avoid presenting equations in the main text and
instead provide them in the Supplemental Material. Although more
mathematically-minded individuals may not appreciate this choice, they
are not our primary target audience, in terms of widening access to
these techniques. Moreover, the availability of the Supplemental
Material means that we can make the main text accessible to a wider
audience whilst not compromising on relevant details. Second, in
addition to relevant equations, we also make use of the Supplemental
Material to expand on further background context whenever relevant.
Third, we have added a new glossary section, which defines and explains
key terms and concepts. Fourth, we have added a section (2.1.3) with
relevant research questions. Fifth, we refer to the CRAN Task View on
survival analysis in section D of the Supplemental Material, where the
reader can find an overview of useful R packages for analysing
time-to-event data. Finally, we formatted all our code according to the
tidyverse style guide. Overall, we feel these choices strike a nice
balance in terms of accessibility. We do not want the paper to become
overly long and technical, but we also want to provide a clear
description of key terms, concepts and relevant equations.
...
Response: We include a new Supplementary Figure 3 to illustrate the
choice of bin width, and better explain the selection of a time window
for analysis (page 25; lines 469-472).
...
Response: We now motivate the use of discrete-time EHA extensively.
First, in section 2.1.2 (pages 9-11). Second, in section D of the
Supplemental Material, where we (a) provide a decision tree to help
select among modeling approaches, and (b) compare the shapes of a few
empirical and parametric distributions.
...
Response: We now include a glossary (section 5; pages 45-47) and example
research questions (section 2.1.3; pages 11-12) in the main text of the
revised ms. Also, we added extra sections to the Supplemental Material,
to increase accessibility further, such as a visual comparison between
the probability mass, survival, cumulative distribution, and hazard
...
overall accessibility, by integrating definitions, examples, rationales,
and analytic guidance into the main text, and by providing a glossary of
key terms.
...
Response: We renamed this section to ``Implications for research in
experimental psychology'' (pages 13-14), and make it clear that RT data
is typically treated as continuous time-to-event data (page 8; lines
135-138). On page 9 (lines 178-180) we mention: ``Even though RT is
typically treated as a continuous variable, here we focus on
discrete-time methods that can be applied after grouping RTs into time
periods (i.e., time intervals or time bins).'' Furthermore, in the
Supplemental Material (sections B) we also mention that the
continous-time RT data are interval censored.
...
Response: We include Supplementary Figure 3 to illustrate how results
are sensitive to the choice of bin width. We also mention that ``Careful
consideration of bin width is important bacause overly small bin widths
will result in erraticly shaped hazard functions, and some bins will
have no events. In contrast, overly large bin widths will result in a
loss of temporal precision (see Supplementary Figure 3)'', on page 14 of
the revised ms (lines 302-305).
...
equals 0 and ca(t) is undefined. In the Note of Table 3, we now mention:
``Conditional accuracy is also undefined for bins without events.''
...
Response: We re-wrote this sentence (page 25; lines 469-472): ``First,
because the first few bins often contain no responses in RT tasks, one
may want to select a smaller time window for analysis compared to the
follow-up period in each trial (see section A of the Supplemental
Material)''
...
Response: We added the section reference numbers in the main text (page
29, lines 547-548).
...
Response: As we make clear in our response to the editor, we spent a
long time considering how to make the paper as accessible as possible,
whilst not compromising on clear descriptions of relevant detail. Please
see the response to the editor for a summary of our revised approach. We
now present the descriptions of RT data, research questions, and
analytic approaches in the main text. Nevertheless, we still opted to
keep the text equation-light, to increase accessibility for
non-mathematical psychologists. We also added various new sections to
the Supplemental Material (sections B, C, and D) and we added a glossary
of key terms in the main text (section 5, pages 45-47). Together, we
believe that these steps improve accessibility whilst also providing
relevant depth where necessary.
...
Response: We followed this suggestion, and explain key RT
characteristics in the main text now (page 8).
...
Response: We revised this paragraph to clearly motivate our choice for
discrete-time EHA (section 2.1.2; pages 9-11) and note that ``there are
no strict cutoffs for when data should be treated as discrete versus
continuous'' (page 10, lines 193-195). Also, in section D of the
Supplemental Material, we provide a decision tree to help select among
modeling approaches and we compare the shapes of a few empirical and
...
Response: We explain SAT analysis in the new Glossary (section 5, pages
45-47).
...
Response: We now mention on page 8 (lines 135-138): ``In RT
tasks,response times are typically measured with a temporal resolution
of 1 millisecond, or higher. As 1 ms is typically short compared to the
range of empirical RT distributions (e.g., a half second), response
times are generally treated as continuous time-to-event data.''

And on page 9 (lines 177-180): ``The type of models employed, and the
definition of hazard, depend on whether one is using continuous or
discrete time units. Even though RT is typically treated as a continuous
variable, here we focus on discrete-time methods that can be applied
after grouping RTs into time periods (i.e., time intervals or time
bins).''

Together with the information in sections B and D of the Supplemental
Material, the distinction between these two approaches should be clear
now.
...
Response: We decided to remove the equations for hazard and conditional
accuracy from the figure (to keep the text equation-light and
accessible), but provide accessible definitions/explanations in (a) the
legend of Figure 1, (b) the main text (pages 10-11), and in the new
glossary (pages 45-47).
...
Response: We removed ``instantaneous'' when talking about risk in
discrete-time.
...
Response: We define key EHA elements in the main text, and in the new
Glossary (pages 45-47).
...
Response: Right censoring is now defined on page 9, and we include the
definitions for right-, left-, and interval-censoring in the new
Glossary (pages 45-47).
