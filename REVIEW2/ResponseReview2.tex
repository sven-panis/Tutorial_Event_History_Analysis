...
concepts. Overall, we feel these choices strike a nice balance in terms
...
Response: We now include a glossary and example research questions in
the main text of the revised ms. Also, we added extra sections to the
Supplemental Material, to increase the accessiblity further.
...
treated as continuous time-to-event data, so that it becomes implicitly
clear that various bin widths can be considered later during analysis.
...
results are sensitive to the choice of bin width. We also mention that
``Overly small bin widths will result in erraticly shaped hazard
functions, and some bins will have no events and thus hazard estimates
of zero; overly large bin widths will result in a too great loss of
temporal information (see Supplementary Figure 3)'', on page 14 of the
revised ms.
