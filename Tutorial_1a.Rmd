...
data <- raw %>% filter(practice==0) # 9240 experimental trials: 6 subj x 70 blocks x 22 trials-per-block
...
map2(paste0("Tutorial_1_descriptive_stats/figures/Plot_for_subject", data_final$pid, "_PanisSchmidt.png"), data_final$plot, ggsave, width = 8, height = 8, dpi = 800)
```
...
data_agg_final <- data_agg_nested %>% 
    ...
    mutate(condacc   = map(censored, calc_ca)) %>%      
    mutate(lifetable_ca = map2(lifetable, condacc, join_lt_ca)) %>%  
    # set haz_yaxis to .7, first_bin_shown to 1, aggregated_data to TRUE, and Nsubj to 6 
