...
To set up a life table, the user has to supply both a width in ms for the time bins, and a censoring time, the last time point for which the trials are considered under investigation (i.e., a cutoff time, or timeout). The goal is to find the optimal bin width and censoring time (both in ms) to portray the effects of interest. Note that all response times must be positive.
...
Load our custom functions. Make sure you specify the same levels and labels for factor condition in the function **plot_eha** (line 164-166 in custom_functions.R) as above (lines 119-122).
