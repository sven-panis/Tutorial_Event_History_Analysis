...
The shape of the prior predictive distribution for 1 bin looks as expected (compare with Figure 16 (row F) in the supplementary material): hazard values below .5 are more likely than hazard values above .5.
...
### Intermezzo. Prior predictive check for model M0c.
...
Finally, perform a ppc for the selected prior distributions for M0c (see lines 369-374), and increase the standard deviation for the normal prior for class "sd" somewhat.
...
After these prior predictive checks, we can increase the standard deviation for class "sd" to 1 to cover many possible inter-individual differences, and fit model M0c to get the posterior distributions.
...
Model_M1d took about 184 minutes to run. Note that duplicate terms in the model formula (e.g., condition) are ignored.
...
Following Heiss (2021), we can plot the expected value of the posterior predictive distribution -- the predicted hazard values -- for the average participant, for each participant in the data set, and for a brand new hypothetical participant.
...
Fourth, six completely new hypothetical participants.

```{r epred-new-pid-6}
epreds_pid_new6 <- dat_M1i %>% 
  data_grid(timebin, prime, pid = 7:12) %>%
  add_epred_draws(model_M1i, 
                  re_formula = NULL, 
                  allow_new_levels=T,    
                  sample_new_levels = "gaussian") %>% 
...
```{r epred-new-pid-6-summary}
epreds_pid_new6 %>% 
  group_by(pid,timebin,prime) %>% 
  median_qi(.width=.95)
```

```{r epred-new-pid-6-plot}
 ggplot(epreds_pid_new6, aes(x=timebin, y=.epred, 
                       fill=prime, color=prime)) +
    stat_lineribbon(point_interval="median_qi",
                    .width=.95,
                    alpha = 0.5) +
    ...
    scale_y_continuous(limits=c(0,1)) +
    labs(y = "predicted hazard") +
    ggtitle("Six new hypothetical participants") +
    facet_wrap(~pid, ncol=3)
```

```{r save-epred-pid-new, eval=F}
ggsave("Tutorial_2_Bayesian/figures/M1i_pred_pid_new_6.png", width = 10, height = 8, dpi = 800)
```
