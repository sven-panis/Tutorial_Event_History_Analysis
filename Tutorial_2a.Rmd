...
Prepare the data file, and specify priors. The skew_normal prior is set for each grand intercept on the cloglog scale, and reflects our prior belief that hazard values above .5 are less likely than values below .5. 
...
### Intermezzo: Prior Predictive Check for M0i. 

Perform a prior predictive check to see if the binary observations generated from the specified prior distribution(s) reflects our prior beliefs. First, sample the prior distributions using sample_prior="only".
...
The shape of the prior predictive distribution looks as expected (compare with Figure 16 (row F) in the supplementary material): hazard values below .5 are more likely than hazard values above .5.
...
After our prior predictive check, we can fit model M0i to get the posterior distributions.
...
Next, sample the prior distributions, and set the variance of the normal prior for class "sd" to a low value, to minimize the effect of the random effects while inspecting the relation between the selected priors for period_9 and period_9_sq and the shape of the prior predictive distributions.
...
```{r save-ppc-M0c, eval=F}
ggsave("Tutorial_2_Bayesian/figures/M0c_ppc.png", width = 10, height = 8, dpi = 800)
```

#
Suppose we believe apriori that hazard functions are flat.
...
priors_M0c_ppc2 <- c(
  ...
  set_prior("normal(0, .001)", class = "sd"),  # set standard deviation to a small value for now
  set_prior("lkj(2)", class = "cor")  
)
```
...
# extract predictions
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0c_prior2, 
                                    newdata = newdata_prior, 
                                    summary=F, 
                                    ndraws=200) %>% 
                mutate(obs = i)
...
Or, suppose we believe that hazard increases with time; More specifically, we assume that cloglog-hazard increases linearly over time.
...
# Generate prior predictive hazard functions for 6 participants
# Specify
N_obs = 300 # increase for smoother results
set.seed(1)
...
# extract predictions
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0c_prior3, 
                                    newdata = newdata_prior, 
                                    summary=F, 
                                    ndraws=20) %>% 
                mutate(obs = i)
...
# plot 200 draws per participant, highlighting three
sel = sample(1:20,3,replace=F)

ggplot(df_pred_haz %>% 
   filter(.draw <= 20), aes(x=period_9, y=pred_haz, group=.draw)) +
   geom_line(color="black") +
   # highlight
   geom_line(data=df_pred_haz %>% filter(.draw ==sel[1]), 
             color="red", linewidth=2) +
...
Or, suppose we believe that hazard first increases with time, and then decreases; 

```{r priors-M0c-ppc2}
priors_M0c_ppc3 <- c(
  set_prior("skew_normal(-1,1,-2)", class = "b", coef = "Intercept"),  
  set_prior("normal(0.6,.2)", class = "b", coef = "period_9"), 
  set_prior("normal(-0,.09)", class= "b", coef="period_9_sq"),   
  set_prior("normal(0, .1)", class = "sd"),  # set standard deviation to a small value for now
  set_prior("lkj(2)", class = "cor")  
)
```
...
Now increase the standard deviation again of the normal prior for class "sd", and fit model M0c to get the posterior distributions.
...
```{r epred-new-pid-summary}
epreds_pid_new %>% 
  group_by(pid,timebin,prime) %>% 
  median_qi(.width=.95)
```

```{r epred-new-pid-plot}
...
[[remove next section and 2 code chunks....]]
