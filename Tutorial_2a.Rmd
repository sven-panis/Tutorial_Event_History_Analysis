...
# create categorical predictor "prime" with index-coding
  ...
  select(pid,trial,trial_c,stage,condition,prime,period,period_9,timebin,d6:d15)
...
There are two ways to implement such a model. First, we can use the index-coding approach, as .. .by McElreath...
...
priors_M0 <- c(
  set_prior("skew_normal(-0.59,1.26,-4.22)", class = "b"), # flat prior for p
  set_prior("normal(0, 1)", class = "sd"),                    
  set_prior("lkj(2)", class = "cor")                        
)
```

Fit the model (M0). old model M0d 
...
Instead of using a general specification of TIME, we can treat TIME as a continuous variable and make assumptions about how hazard changes over time within a trial. For example, we might assume that hazard changes in a linear + quadratic fashion over timebins within a trial.
...
The next model we fit also includes our predictor variable prime-target congruency.
First, we can use index coding for the categorical predictor.


Second, if we do not want to use index coding, assume that hazard can vary in a linear+quadratic fashion over time, and relax the proportionality assumption, then we can fit the following model.

## Model M2.

The next model we fit also includes our predictor trial number.

First, we can categorise the continuous predictor trial number by grouping them in one of three "stages" of the experiment (stage 1 = trials 1 to 500; stage 2 = trials 501 to 1000; stage 3 = trials 1001 and later) and use index coding.


Second, we can treat trial number and bin number as continuous variables, and center them.
