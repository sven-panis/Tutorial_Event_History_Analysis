...
model_M0_prior <-                     # wrong data file name
   brm(data = M0_data,
       ...
       seed = 12, init = "0",
       sample_prior = 'only',
       file = "Tutorial_2_Bayesian/models/model_M0_prior")
```

```{r}

#pp_check(model_M0_prior, stat="mean")

#http://bruno.nicenboim.me/bayescogsci/
  
# prior predictive check for 1 time bin
cloglog_predictive_distribution <-
  function(samples, N_obs) {
      ...
      for (i in seq_along(samples)) {
        ...
        df_pred <- bind_rows(df_pred,
                             tibble(trial = 1:N_obs,
                                    y_pred = rbernoulli(N_obs, 1-exp((-1)*exp(cloglog_haz))),
                                    iter = i))
    }
    df_pred # return
  }
# simulate prior hazard values for 1 bin
N_samples <- 3000
N_obs <- 100 #nrow(M0_data)
#cloglog_samples <- rskew_normal(N_samples,-0.59,1.26,-4.22) # flat prior for cloglog intercept
cloglog_samples <- rskew_normal(N_samples,-1,1,-2) # weakly informative prior for cloglog intercept

prior_pred <- cloglog_predictive_distribution(cloglog_samples,N_obs)

prior_pred_haz <- prior_pred %>% group_by(iter) %>% summarise(pred_haz = mean(y_pred))
...
```{r}
# Generate prior predictive samples for 10 bins 
bins_cloglog_predictive_distribution <-
  function(samples_bin, N_obs, N_bins) {
    # empty data frame with headers:
    df_pred <- tibble(trial = numeric(0),
                      y_pred = numeric(0),
                      iter = numeric(0),
                      bin = numeric(0))
    # i iterates from 1 to the length of samples,
    # b interates from 1 to N_bins
    for( b in 1:N_bins){
      for (i in 1:dim(samples_bin)[2]) {
        cloglog_haz <- samples_bin[b,i]
        df_pred <- bind_rows(df_pred,
                             tibble(trial = 1:N_obs,
                                    y_pred = rbernoulli(N_obs, 1-exp((-1)*exp(cloglog_haz))),
                                    iter = i,
...
# simulate prior hazard functions for 10 bin
N_samples <- 40
N_obs <- 100
N_bins <- 10
#cloglog_samples <- rskew_normal(N_samples,-0.59,1.26,-4.22) # flat prior for cloglog intercept
cloglog_samples_bin <- data.frame()
for(b in 1:N_bins){
  cloglog_samples_bin[b,1:N_samples] <- rskew_normal(N_samples,-1,1,-2) # weakly informative prior for cloglog intercept
}

prior_pred <- bins_cloglog_predictive_distribution(cloglog_samples_bin,N_obs,N_bins)

prior_pred_haz <- prior_pred %>% group_by(iter,bin) %>% summarise(pred_haz = mean(y_pred))

# plot 
ggplot(prior_pred_haz, aes(x=bin, y=pred_haz, group=iter)) +
  #geom_histogram(binwidth=.01) +
  geom_line(color="black") +
  geom_line(data = prior_pred_haz %>% filter(iter==4), color="red", linewidth = 2) +
  geom_line(data = prior_pred_haz %>% filter(iter==18), color="blue", linewidth = 2) +
...
newdata_prior = tibble(timebin = c(6:15)) %>% 
                            expand_grid(pid = 1:6)
N_obs = 100
...
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0_prior, newdata = newdata_prior, summary=F, ndraws=NULL)
  prior_pred <- prior_pred %>% mutate(obs = i)
  df_pred <- bind_rows(df_pred, prior_pred)
}
head(df_pred)

# calculate hazard per draw (average across obs)
df_pred_haz <- df_pred %>% group_by(pid,timebin,.draw) %>% summarise(pred_haz = mean(.prediction))
summary(df_pred_haz)

# plot 40 draws per participant
ggplot(df_pred_haz %>% filter(.draw < 21), aes(x=timebin, y=pred_haz, group=.draw)) +

   geom_line(color="black") +
   geom_line(data=df_pred_haz %>% filter(.draw == 20), color="red", linewidth=2) +
...
## Intermezzo: Prior predictive checks
...
df_pred <- tibble(period_9 = integer(0),
                  period_9_sq = double(0),
                  period_9_cu = double(0),
                  pid = integer(0),
                  .row = integer(0),
                  .chain = integer(0),
                  .iteration = integer(0),
...
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0c_prior, newdata = newdata_prior, summary=F, ndraws=NULL)
  prior_pred <- prior_pred %>% mutate(obs = i)
  df_pred <- bind_rows(df_pred, prior_pred)
}
head(df_pred)
...
# calculate hazard per draw (average across obs)
df_pred_haz <- df_pred %>% group_by(pid,period_9,.draw) %>% summarise(pred_haz = mean(.prediction))
summary(df_pred_haz)

# plot 40 draws per participant
ggplot(df_pred_haz %>% filter(.draw < 2), aes(x=period_9, y=pred_haz, group=.draw)) +

   geom_line(color="black") +
   geom_line(data=df_pred_haz %>% filter(.draw == 17), color="red", linewidth=2) +
...
dat <- tibble(x = -3:6,
              x2 = x^2,
              x3 = x^3,
              cloglog1 = -1.8   + 0.3*x   - 0.1*x2   + 0.009*x3,
              cloglog2 = -1     + 0.4*x   - 0.1*x2   + 0.007*x3,
              cloglog3 = -3     - 0.1*x   + 0.12*x2  - 0.002*x3,
              cloglog4 = -0.40  + 0.2*x   - 0.04*x2  + 0.003*x3,
...
```{r}
tibble(cloglog = rnorm(1000, .1, .1)) %>%
  ggplot(aes(x=cloglog)) +
  geom_density()
```
...
#set_prior("normal(0.5,.5)",class="b",coef="period_9"),
#set_prior("normal(-.1,.2)",class="b",coef="period_9_sq"),   # Np3

#set_prior("normal(0.1,.08)",class="b",coef="period_9"), # ok !!!
#set_prior("normal(0,.06)",class="b",coef="period_9_sq"),

set_prior("normal(0.1,.1)",class="b",coef="period_9"), # ok !!!
set_prior("normal(0,.06)",class="b",coef="period_9_sq"),
#set_prior("normal(0,.003)",class="b",coef="period_9_cu"),


#set_prior("skew_normal(0.3,0.1,2)",class="b",coef="period_9"), #skNp3
#set_prior("skew_normal(0,0.1,0)",class="b",coef="period_9_sq"),
...
model_M0_lin_prior454 <-
   brm(data = M0c_data,
       family = binomial(link="cloglog"),
       event | trials(1) ~ 0 + Intercept + period_9 + period_9_sq  +
                          (0 + Intercept + period_9 + period_9_sq  | pid),
       prior = priors_M0c_prior,
       chains = 4, cores = 4, iter = 3000, warmup = 1000,
       control = list(adapt_delta = 0.999, step_size = 0.04, max_treedepth = 12),
       seed = 12, init = "0",
       sample_prior = "only",
       file = "Tutorial_2_Bayesian/models/model_M0_lin_prior454")
```

```{r}
#model_M0_lin_prior448 <- readRDS("Tutorial_2_Bayesian/models/model_M0_lin_prior448.rds")

post <- as_draws_df(model_M0_lin_prior454) %>% 
  select(-lp__) %>% 
  as_tibble()
...
# plot
p_tidy_fixed <- ggplot(tidy_fixed, aes(x = name, y = value, fill = name)) +  
  stat_halfeye(alpha=0.7, point_interval = "median_qi",.width=c(.8,.99) ) 
p_tidy_fixed
...
```{r}
#model_M0_lin_prior14 <-readRDS("Tutorial_2_Bayesian/models/model_M0c_lin_prior14.rds")               
# Generate prior predictive hazard functions for 6 participants
newdata_prior = tibble(period_9 = c(-3:6)) %>% 
  mutate(period_9_sq = period_9^2) %>%
  expand_grid(pid = 1:6)
...
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0_lin_prior454, 
...
# plot 40 draws per participant
sel = sample(1:8000,3,replace=F)

ggplot(df_pred_haz %>% 
   filter(.draw <639), aes(x=period_9, y=pred_haz, group=.draw)) +
   geom_line(color="black") +
...
epreds_pid_new <- 
  posterior_epred(model_M1, 
                  newdata = expand_grid(timebin=6:15,
                                        prime=1:3,
                                        pid=7), 
                  re_formula = NULL, 
                  allow_new_levels=T, 
  ...
  pivot_longer(V1:V30,
               values_to = ".epred", 
               names_to = "condition") %>%
  mutate(prime = factor(rep(c(1,2,3), 10*8000), 
                        levels=c(1,2,3),
                        labels=c("blank","congruent","incongruent")),
         timebin = factor(rep(rep(c(6:15),each=3),8000),
                          levels=c(6:15))) %>%
  ungroup()
```
...
What can we conclude from model M1 about our research question, i.e., the temporal dynamics of the effect of prime-target congruency on RT? In other words, in which of the 40-ms time bins between 200 and 600 ms after target onset does changing the prime from blank to congruent or incongruent affect the hazard of response occurrence (for a prime-target SOA of xxx ms)?

If we want to study the average effect of prime type on hazard, uncontaminated by inter-individual differences, we can base our conclusion on the grand AMEs of congruent and incongruent primes in each bin. The contrast "congruent minus blank" was estimated to be 0.0925 hazard units in bin 6 (95% CrI = [0.0411, 0.183]), and 0.152 hazard units in bin 7 (95% CrI = [0.0489, 0.262]). For the other bins, the 95% credible interval contained zero.
...
pabove <- epreds_grand_diffs %>% 
  ...
  summarize(prop_above = mean(pred_haz > 0)) %>% 
  arrange(contrast,timebin)

pbelow <- epreds_grand_diffs %>% 
  group_by(timebin,contrast) %>%
  summarize(prop_below = mean(pred_haz < 0)) %>% 
  arrange(contrast,timebin)
...
[[todo or not todo]]
Plot of posterior distributions of sigma(timebinX:primeY) : check size differences between timebins and prime types? -> these are also population-level effects

If we want to focus more on inter-individual differences, we can study the subject-specific AMEs in Figure XX. While most participants show the grand AMEs, three participants (1, 2, and 3) show a negative difference for the contrast "congruent minus incongruent" in bin (360,400] - subject 2 also in bin (320,360]. This dip is also present in the AMEs for a brand new hypothetical participant.

Future studies could (a) increase the number of participants to estimate the proportion of "dippers" in the subject population, and/or (b) try to explain why this dip occurs. For example, Panis and Schmidt (ref) concluded that active, top-down, task-guided response inhibition effects emerge around 360 ms after the onset of the stimulus following the prime (here:the target). Such a top-down inhibitory effect might exist in our priming data set, because after some time participants will learn that the first stimulus is not the one they have to respond to; To prevent a premature overt response to the prime they thus might gradually increase a global response threshold during the remainder of the experiment, which could result in a lower hazard in congruent trials compared to blank trials, for bins after ~360 ms, and towards the end of the experiment.
Interestingly, all subjects show a tendency in their mean difference (congruent minus blank) to "dip" around that time. Therefore, future modeling efforts could incorporate the trial number into the model formula, in order to also study how the effects of prime type on hazard change on the long experiment-wide time scale, next to the short trial-wide time scale (TIME). In Tutorial_2a.Rmd we provide a number of model formula that should get you going.
