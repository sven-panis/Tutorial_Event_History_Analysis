...
Plot of posterior distributions of sigma(timebinX:primeY) : check size differences between timebins and prime types?
...
Interestingly, all subjects show a tendency in their mean difference (congruent minus blank) to "dip" around that time. Therefore, future modeling efforts could incorporate the trial number into the model formula, in order to also study how the effects of prime type on hazard change on the experiment-wide time scale, next to the trial-wide time scale (TIME). In Tutorial_2a.Rmd we provide a number of model formula that should get you going.
