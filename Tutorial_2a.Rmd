...
dat <- tibble(x = -3:6,
              ...
              cloglog2 = -1     + 0.5*x   - 0.1*x2   + 0.004*x3,
...
model_M0_lin_prior448 <-
   brm(data = M0c_data,
       ...
       seed = 12, init = "0",
       sample_prior = "only",
       file = "Tutorial_2_Bayesian/models/model_M0_lin_prior448")
```

```{r}
post <- as_draws_df(model_M0_lin_prior448) %>% 
  select(-lp__) %>% 
  as_tibble()
...
# plot
p_tidy_fixed <- ggplot(tidy_fixed, aes(x = value, y = name)) +  
...
N_obs = 150 # increase for smoother results
...
for(i in 1:N_obs){
  prior_pred <- add_predicted_draws(model_M0_lin_prior448, 
...
ggplot(df_pred_haz %>% 
   filter(.draw <7639), aes(x=period_9, y=pred_haz, group=.draw)) +
   geom_line(color="black") +
   geom_line(data=df_pred_haz %>% filter(.draw ==7418), 
             color="red", linewidth=2) +
  geom_line(data=df_pred_haz %>% filter(.draw == 6001), 
