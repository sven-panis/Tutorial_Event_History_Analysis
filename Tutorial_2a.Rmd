...
We also plot the logit and cloglog link functions (section 6), and illustrate how various prior distributions for an intercept on the logit and cloglog scales look on the original probability (i.e., discrete-time hazard) scale (section 7). 

# 1. Load the libraries that we will be using.

```{r load-pkg, results='hide'}
pkg <- c(
  "cmdstanr", "standist", "tidyverse", "RColorBrewer", "patchwork",
...
Prepare the data file, and specify priors. The skew-normal prior is set for each grand intercept on the cloglog scale, and reflects our prior belief that higher hazard values (above .5) are less likely than lower values (below .5). This prior is plotted in row F of Supplementary Figure 3 (section F in the Supplemental Material).
...
M0d took about 77 minutes using the same MacBook Pro.
...
Model_M0c took about 19 minutes to run using the same MacBook Pro.
...
Model M1i took about 124 minutes to fit using the same MacBook Pro.
...
check_priors_M1d <- get_prior(
  formula = event ~ 0 + Intercept +
    condition * period_9 +
...
Because TIME is centered on bin 9, the Intercept represents the estimated cloglog-hazard in bin 9 for the blank prime condition. Model_M1d took about 184 minutes to run using the same MacBook Pro.
...
```{r model-weights}
model_weights(model_M0i, model_M0d, model_M0c, model_M1i, model_M1d, weights = "loo") |>
  round(digits = 3) |>
  format(nsmall = 2)

model_weights(model_M0i, model_M0d, model_M0c, model_M1i, model_M1d, weights = "waic") |>
  round(digits = 3) |>
  format(nsmall = 2)
```
...
p_tidy_fixed <- ggplot(tidy_fixed, aes(x = timebin, y = value, fill = condition)) +
  ...
  labs(
    title = "Posterior distributions for population-level\neffects in Model M1i",
    x = "time bin (upper bound in ms)", y = "cloglog-hazard"
  ) +
...
p1 <- ggplot(epreds_grand, aes(
  ...
  labs(x = "time bin (upper bound in ms)", y = "predicted hazard")
...
tidy_fixed |>
  ...
  stat_lineribbon(aes(linetype = condition),
    point_interval = "median_qi",
    .width = .95,
    alpha = 0.5
  ) +
...
epreds_grand_diffs <- epreds_grand |>
  pivot_wider(
    id_cols = c(timebin, .draw),
    names_from = "prime",
    values_from = ".epred"
  ) |> # 80000 x 5
  mutate(
    `congruent minus blank` = congruent - blank,
    `incongruent minus blank` = incongruent - blank
  ) |>
  select(-c(blank, congruent, incongruent)) |>
  pivot_longer(
    cols = c(`congruent minus blank`, `incongruent minus blank`),
    names_to = "contrast",
    values_to = "diff"
  )
...
p11 <- ggplot(epreds_grand_diffs, aes(
  x = timebin, y = diff,
  fill = contrast
)) +
  stat_lineribbon(
  ...
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "time bin (upper bound in ms)", y = "difference in predicted hazard") +
  facet_wrap(~contrast)
...
post |> # 8000 x 30 = 240000
  ...
  mutate(
    bin6_cb = `b_timebin6:prime2` - `b_timebin6:prime1`,
  ...
  pivot_longer(
    cols = bin6_cb:bin15_ib,
    names_to = "condition",
    values_to = "diff"
  ) |>
  mutate(
    bin = str_sub(condition, 4, 5),
...
p22 <- ggplot(epreds_pid_diffs, aes(
  x = timebin, y = diff,
...
ggsave("Tutorial_2_Bayesian/figures/Revision_results_hazard.png", width = 14, height = 20, dpi = 800)
```
...
There are thus two phases of performance for the average person between 200 and 600 ms after target onset. In the first phase, the addition of a congruent or incongruent prime stimulus increases the hazard of response occurrence compared to blank prime trials in the time period 200-240 ms. In the second phase, only the incongruent prime decreases the hazard of response occurrence compared to blank primes, in the time period 320-440 ms. The sign of the effect of incongruent primes on the hazard of response occurrrence thus depends on how much waiting time has passed since target onset.
...
If we want to focus more on inter-individual differences, we can study the subject-specific contrasts. Note that three participants (1, 2, and 3) show a negative difference for the contrast "congruent minus incongruent" in bin 400 -- subject 2 also in bin 360. 
...
pr1 <- tibble(prior = rnorm(1e6, mean = 0, sd = 4)) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -13, y = 60000, label = "N(0,4)",
    color = "red", size = 4
  ) +
...
pr2 <- tibble(prior = rnorm(1e6, mean = 0, sd = 2)) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -6, y = 60000, label = "N(0,2)",
    color = "red", size = 4
  ) +
...
pr3 <- tibble(prior = rt(1e6, df = 7.61) * 1.57) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -12, y = 140000, label = "t(7.61,0,1.57)",
    color = "red", size = 4
  ) +
...
pr4 <- tibble(prior = rnorm(1e6, mean = 0, sd = 1)) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -3, y = 60000, label = "N(0,1)",
    color = "red", size = 4
  ) +
...
pr5 <- tibble(prior = rskew_normal(1e6, mu = -0.59, sigma = 1.26, alpha = -4.22)) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -5.5, y = 55000, label = "skew_N(-0.59,1.26,-4.22)",
    color = "red", size = 4
  ) +
...
pr6 <- tibble(prior = rskew_normal(1e6, mu = -1, sigma = 1, alpha = -2)) |>
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  annotate(
    geom = "text", x = -5, y = 55000, label = "skew_N(-1,1,-2)",
    color = "red", size = 4
  ) +
...
# apply the function
prior_pred <- cloglog_predictive_distribution(
  cloglog_samples,
  N_obs
)
...
The shape of the prior predictive distribution for 1 bin looks as expected (compare with Supplementary Figure 3 (row F) in section F of the Supplemental Material): hazard values below .5 are more likely than hazard values above .5.
...
ggplot(
  data = df_pred_haz |> filter(.draw < 200),
  aes(x = timebin, y = pred_haz, group = .draw)
) +
  geom_line(color = "black") +
  geom_line(
    data = df_pred_haz |> filter(.draw == 20),
    color = "red",
    linewidth = 2
  ) +
  scale_y_continuous(limits = c(0, 1)) +
...
plot_ppc <- function(fit_prior, N_samples, N_obs, new_data_prior) {
  ...
  ggplot(df_pred_haz |>
    ...
    # highlight
    geom_line(
      data = df_pred_haz |> filter(.draw == sel[1]),
      color = "red", linewidth = 2
    ) +
    geom_line(
      data = df_pred_haz |> filter(.draw == sel[2]),
      color = "green", linewidth = 2
    ) +
    geom_line(
      data = df_pred_haz |> filter(.draw == sel[3]),
