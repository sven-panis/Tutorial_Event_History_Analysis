...
wordcount         : "10167 (body) + 1742 (references) + 3458 (body supplemental material) + 539 (refs suppl. mat.)"
bibliography      : ["refs_manuscript.bib", "r-references.bib"]
floatsintext      : yes
linenumbers       : yes
draft             : no
mask              : no
...
# R references
#r_refs(file = "r-references.bib") # creates a .bib-reference file
my_r_citation <- cite_r(file = "r-references.bib", 
                        footnote = TRUE) # use in main text to cite R 
```
...
In experimental psychology, it is standard practice to analyse response times (RTs), saccade latencies, and fixation durations by calculating average performance across a series of trials. Such comparisons between means have been the workhorse of experimental psychology over the last century, and have had a substantial impact on theory development as well as our understanding of the structure of cognition and brain function. Indeed, the view that mean values represent truth and variations around the mean are error is deeply ingrained in experimental psychology [@bolgerCausalProcessesPsychology2019]. However, differences in mean RT conceal important pieces of information, such as when an experimental effect starts, how it evolves with increasing waiting time, and whether its onset is time-locked to other events [@panisHowCanWe2020; @panisNeuropsychologicalEvidenceTemporal2017; @panisStudyingDynamicsVisual2020; @panisTimecourseContingenciesPerceptual2009; @panisWhatShapingRT2016; @panisWhenDoesInhibition2022; @wolkersdorferTemporalDynamicsSequential2020].

As a simple illustration, Figure 1 summarises simulated data for one subject that shows how comparing means between two conditions can conceal the shapes of the underlying RT and accuracy distributions. Indeed, compared to the aggregation of data across trials (Figure 1A), a distributional approach offers the possibility to reveal the time course of psychological states (Figure 1B). Here we apply a distributional method known as **discrete-time event history analysis (EHA)** extended with **speed-accuracy tradeoff analysis (SAT)**. All terms in bold can be found in the glossary (section 2.4). 

(ref:descr-fig1-caption) Simulated single-subject data showing mean performance versus a distributional analysis. (A) The mean RT (top) and overall accuracy (bottom) for two conditions are plotted. Two hundred trials are simulated in each condition. (B) The **discrete-time hazard functions** (top) and **conditional accuracy functions** (bottom) are plotted for the same data. The first second after target stimulus onset (time zero) is divided in ten time bins of 100 ms (indexed by t = 1 to 10). The hazard and conditional accuracy estimates are plotted at the upper bound of each time bin. Hazard for bin t is the conditional probability of event occurrence in bin t given no events in earlier bins. Conditional accuracy for bin t is the conditional probability of a correct response given response occurrence in bin t. The mathematical definitions of discrete-time hazard and conditional accuracy are provided in section X of the Supplemental Material. Error bars represent +/- 1 standard error of the mean (A) or proportion (B). 

```{r plot1, fig.cap = "(ref:descr-fig1-caption)", out.width="90%"}
knitr::include_graphics("../sims/figures/Fig1_revision1.jpeg")
```

For example, Figure 1B shows a first state (up to 400 ms after target onset) for which the early upswing in the hazard functions of response occurrence is equal for both conditions, and the emitted responses are always correct in condition 1 and always incorrect in condition 2. In a second state (400 to 500 ms), the hazard of response occurrence is higher in condition 1, and conditional accuracies are close to .5 in both conditions. In a third state (>500 ms), the effect disappears in hazard, and all conditional accuracies are equal to 1. Note that we will always refer to a time bin by its upper bound. For example, time bin "500" in Figure 1B refers to the time interval running from 400 ms to 500 ms, with the lower bound of 400 ms excluded, and the upper bound of 500 ms included. Importantly from a face-validity perspective, this pattern of simulated data can be seen in the experimental psychology literature [@panisWhenDoesInhibition2022; @panisWhatShapingRT2016; @panisHowCanWe2020].

Why does this matter for research in psychology? First, as illustrated below, for many psychological questions, differences and similarities between the shapes of the RT distribution in two experimental conditions can be theoretically meaningful by leading to more fine-grained understanding of psychological processes. Second, extracting absolute timing information is useful not only for the interpretation of experimental effects under investigation, but also for cognitive psychophysiology and computational model selection [@panisAnalyzingResponseTimes2020]. Because EHA adds a relatively under-used but ever-present dimension -- the passage of time -- to the theory building toolkit, it thus constitutes one possible response to the recent call for a temporal science of behavior [@ABNEY2025].

## 1.2 Aims 

Our ultimate aim in this paper is twofold. First, we want to inform readers of the many benefits of using EHA when dealing with psychological RT data, and other types of time-to-event data. Second, we want to provide a set of practical tutorials, which provide step-by-step instructions on how you actually perform a (single event) discrete-time EHA on RT data, as well as a complementary discrete-time SAT analysis on timed accuracy data in case of choice RT data (Figure 1B).

Even though EHA is a widely used statistical tool and there already exist many excellent textbooks [@singerAppliedLongitudinalData2003; @allisonDiscreteTimeMethodsAnalysis1982; @blossfeldTechniquesEventHistory2002; @hosmerAppliedSurvivalAnalysis2011; @teachmanAnalyzingSocialProcesses1983; @box-steffensmeierEventHistoryModeling2004; @millsIntroducingSurvivalEvent2011] and tutorials [@allisonSurvivalAnalysisUsing2010; @landesIntroductionEventHistory2020; @elmerModelingCategoricalTimetoevent2023; @lougheedMultilevelSurvivalAnalysis2019; @stoolmillerIntroductionUsingMultivariate2015; @stoolmillerModelingHeterogeneitySocial2006], we are not aware of any tutorials that are aimed specifically at psychological RT (+ accuracy) data, and which provide worked examples of the key data processing and Bayesian multilevel regression modelling steps. 
...
In what follows, the paper is organised in three main sections. In Section 2, we provide a brief overview of EHA, including basic concepts, modeling approaches, and the new types of research questions that EHA allows to answer about behavioral dynamics.
In Section 3, we outline a series of tutorials, which are written in the R programming language and publicly available on our Github page (https://github.com/sven-panis/Tutorial_Event_History_Analysis), along with all of the other code and material associated with the project. The tutorials provide hands-on, concrete examples of key parts of the analytical process, such as data wrangling, plotting descriptive statistics, model fitting and planning future studies, so that others can apply EHA to their own time-to-event data measured in RT tasks. 
In Section 4, we discuss the overall strengths and weaknesses of the approach for researchers in experimental psychology, compared to orthodox methods such as Analysis-of-variance, linear regression, and delta-plots (ref).

# 2. What is event history analysis and why is it relevant to research in experimental psychology?

In this section, we want to provide an intuition regarding how EHA works in general, as well as in the context of experimental psychology. We assume readers are familiar with R and (logistic) regression equations. For those who want more detailed treatment of regression equations, we refer the reader to several excellent textbooks [@gelmanRegressionOtherStories2020; @winterStatisticsLinguistsIntroduction2019]. We decided to keep this text intentionally equation-light, and provide additional information and mathematical definitions in the Supplementary Material, including relevant regression equations (see section E of the Supplemental Material). 

## 2.1 A brief introduction to event history analysis

EHA is a class of statistical approaches to study the occurrence and timing of events, such as death, disease onset, marriages, arrests, and job terminations [@allisonSurvivalAnalysisUsing2010]. EHA is known by various labels, such as survival analysis, hazard analysis, duration analysis, failure-time analysis, and transition analysis [@singerAppliedLongitudinalData2003]. In this paper, we choose to use the term EHA throughout. An **event** is defined as a qualitative change -- a transition from one discrete state to another -- that can be situated in time. Typical events of interest in cognitive science include manual button presses, vocal response onsets, saccadic eye-movement onsets, fixation offsets, and spatial boundary crossings when measuring reach movement trajectories, for example in mouse-tracking studies (refs: mouse + KL omar). While some events can only happen once (e.g., death, onset chronic disease), most events of interest for cognitive scientists can happen repeatedly to the same individual.

### 2.1.1 EHA concepts applied to RT tasks / Terminology and minimum requirements for EHA

To apply EHA one also must define time point zero (e.g., stimulus onset), and measure the passage of time between time point zero and event occurrence in discrete or continuous time units. In RT tasks, response times are typically measured with a temporal resolution of 1 millisecond, or higher. As 1 ms is typically short compared to the range of empirical RT distributions (e.g., a half second), response times are generally treated as *continuous* time-to-event data.

In standard applications, each individual can experience the event only once. In experimental psychology and cognitive neuroscience, however, most events are repeatable, and typically, events are repeatedly measured within experimental trials, where each individual completes multiple independent trials and each trial yields one time-to-event observation.
For such **repeated measurements** or trial-based events, the **observation period** refers to the entire experimental session during which multiple independent trials (or **episodes**) are completed. In each trial, the individual is "at risk" for event occurrence as long as the participant is eligible to experience the event. In other words, a participant is considered "at risk" when (a) they have not yet experienced the event of interest in a trial, and (b) they are still under observation.

In section A of the Supplemental Material we visualize various types of time-to-event data sets that are typically obtained in detection, discrimination, and bistable perception tasks.
...
These minimal requirements are fulfilled by the RT data obtained in single-button detection tasks, where the time-to-response is repeatedly measured in different trials in the same individual. -->


### 2.1.2 Modeling approaches / Types of EHA


There are several different modeling approaches available between which one can select when analyzing a time-to-event data set. The type of models employed, and the definition of hazard, depend on whether one is using continuous or discrete time units. Even though RT is typically treated as a continuous variable, here we focus on discrete-time methods that can be applied after grouping RTs into time periods (or intervals, bins). Discrete-time methods are particularly useful when (a) the shape of the hazard function is unknown and the analyst wants to estimate it flexibly without parametric assumptions, and (b) the analyst is interested in examining and interpreting the shape of the **baseline hazard function**. In section X of the Supplementary Material we provide some examples to justify our choice of discrete-time modeling.
Also, because continuous forms of EHA require a lot of data to reliably estimate the shape of the continuous-time hazard (rate) function [@bloxomEstimatingResponseTime1984; @luceResponseTimesTheir1991; @vanzandtHowFitResponse2000], discrete-time methods seem ideal for dealing with typical psychological RT data sets for which there are less than ~200 trials per condition per participant as they allow a tradeoff between temporal resolution (i.e., bin width) and the number of repeated measurements per condition per participant [@panisAnalyzingResponseTimes2020].
Moreover, as indicated by @singerAppliedLongitudinalData2003, learning discrete-time EHA methods first will help in learning continuous-time methods, so it seems like a good entry point into EHA. Note that the choice is ... see reviewer 3....

To apply discrete-time EHA, one divides the within-trial time (or episode) in discrete, contiguous time bins indexed by t (e.g., t = 1 to 10; Figure 1B).
...
In other words, it reflects the risk that the event occurs in the current bin t, given that it has not yet
...
The mathematical definitions of both these and other discrete-time functions are given in section X of the Supplemental Material.

###2.1.3 Research Questions

EHA allows to answer new research questions on behavioral dynamics in experimental paradigms to study cognition:

- What is the shape of the hazard function of response occurrence? In other words, what is the effect of the passage of time on the within-trial (or across-bin) time scale on the hazard estimate? Does the hazard function increase to a plateau, or does it increase to a peak and then decrease? Is it flat in the right tail?

- Does an experimental manipulation affect the early or late part of the distribution? Do experimental effects increase or decrease in strength over time within a trial, or are they restricted only to a short time period within a trial?

- Can we identify cognitive states in the h(t) + ca(t) patterns? For example, based on Figure 5C and 5D, this participant transitions through at least three motor states during a trial: a first state running from -160 to -40 ms, reflecting the automatic response to the peripheral cue; a second state running from 0 to 160 ms, where conditional accuracies reverse; and a third state where conditional accuracy increases to 1 and the hazard functions display the inhibition-of-return effect (i.e., higher hazard for invalid compared to valid cueing). 

- Do time scales interact? In other words, is the shape of the hazard function within a trial changing as time passes by on longer time scales? For example, some effects might increase, or decrease in size and disappear, over the course of an experiment (the across-trial time scale). Another example is that people typically speed up during the course of an experiment, but is this reflected in changes across the whole distribution or only in the left or right tail? 

- Do predictor variables interact? Do time scales interact with predictor variables? Is the effect of a continuous predictor on hazard linear or nonlinear? 

- Are experimental effects time-locked to certain stimuli (ref)?

- Are there inter-individual differences in the shape of the hazard function (ref)?

- Do **time-varying predictors** (eye gaze position, eye dilation, heart rate, parietal-frontal synchronization, ...) affect the risk of event occurrence?

## 2.2 Benefits of event history analysis for research in experimental psychology

Statisticians and mathematical psychologists recommend focusing on the hazard function when analyzing time-to-event data for various reasons [@holdenDispersionResponseTimes2009; @townsendTruthConsequencesOrdinal1990; @luceResponseTimesTheir1991], as explained in section G of the Supplemental Material.
Here we highlight three benefits that we think are relevant to the domain of experimental psychology. 
...
Will the temporal effect patterns align nicely with electroencephalography (EEG) findings [@panisWhenDoesInhibition2022]?
...
The fact that EHA can deal with right-censoring, therefore, presents a analytical strength of the approach compared to many common approaches in experimental psychology (e.g., ANOVA, linear regression, delta plots). Other types of censoring include **left censoring** and **interval censoring**.
...
The upshot is that one general analytical approach, which holds several potential advantages, is widely applicable to many substantive use-cases in the RT domain of experimental psychology, irrespective of the analyst's current view on the nature of cognition [@barackTwoViewsCognitive2021].
...
\begin{description}
\item[Discrete-time event history analysis (EHA)] A statistical method for analyzing the occurrence and timing of events when time is measured using (or grouped into) discrete intervals (e.g., years, months, survey waves). Commonly used when events are naturally recorded at specific measurement occasions, but also applicable to continuously measured time after grouping into intervals. Discrete-time methods are particularly useful when: (a) the shape of the hazard function is unknown and the analyst wants to estimate it flexibly without parametric assumptions, and (b) the analyst is interested in examining and interpreting the shape of the baseline hazard function (in contrast to Cox regression, which leaves the baseline hazard unspecified).

\item[Speed-accuracy tradeoff analysis] A speed-accuracy tradeoff (SAT) analysis with conditional accuracy functions (CAFs) examines how an individual's accuracy changes at different speeds for specific response time intervals, often under different conditions like varying deadlines or payoffs. This approach moves beyond simple average error rates to show how accuracy is not uniform across all speeds.

\item[Discrete-time hazard Function] The pattern of hazard probabilities over time, showing how the risk of event occurrence changes across time periods.

\item[Conditional accuracy function (CAF)] A plot that shows the probability of an accurate response as a function of the response time (RT) or a specific time period.

\item[Event] The outcome of interest whose occurrence and timing are being studied (e.g., button press, job loss, marriage, death, graduation). Events can be repeatable or non-repeatable.

\item[Time Period (Bin, Interval)] The discrete unit of time used in the analysis (e.g., year, month, week, second). Each observation contributes one record per time period at risk.

\item[Hazard (Hazard Probability)] The conditional probability that an individual will experience the event in a particular time bin or time period (of a trial or episode), given that they have not yet experienced it by the beginning of that period. Denoted as h(t) or P(T=t|T$\geq$t).

\item[Continuous-Time Hazard (Hazard Rate, Instantaneous Hazard)] The instantaneous rate at which events occur at time point t, given survival to time t. Formally defined as the limit h(t) = lim($\Delta t \to 0$) P(t $\leq$ T < t+$\Delta t$ | T$\geq$t)/$\Delta t$. Unlike discrete-time hazard (a conditional probability), continuous-time hazard is a rate that can exceed 1. Denoted as $\lambda(t)$.

\item[Observation Period] The span of time during which an individual is under observation or follow-up in the study. The observation period may encompass one or more episodes. For non-repeatable events, the observation period and episode are typically the same. For repeatable events in natural history (e.g., multiple job changes), the observation period can contain multiple sequential episodes. For trial-based events, the observation period refers to the entire experimental session during which multiple independent trials (episodes) are completed. The observation period ends when the event occurs (for non-repeatable events), the study ends, or the individual is lost to follow-up.

\item[Episode (Spell, Risk Episode)] The duration from the start of observation or from a previous event until either the event occurs or observation ends. Also called a "spell" (particularly in labor economics) or "risk episode." An episode represents one complete at-risk period being studied. Within each episode, data may be collected at multiple discrete time points (e.g., monthly or yearly intervals) — these are the time periods or measurement occasions. In discrete-time analysis, each episode is typically divided into multiple person-period observations, where each person-period is the actual unit of analysis for model estimation. In repeatable event contexts, individuals may contribute multiple episodes. For example, one person's marriage might be a single episode observed over 10 yearly measurements (resulting in 10 person-period observations), while in a trial-based experiment, one person completes 100 episodes (trials), each potentially lasting several time periods.

\item[Repeated Measurements (Trial-Based Events)] Events that are repeatedly measured within experimental trials, where each individual completes multiple independent trials and each trial yields one time-to-event observation (e.g., reaction time tasks). Unlike recurrent events in natural history, these trials are typically designed to be independent realizations of the same process, though individual-level random effects may be needed to account for within-subject correlation. In this context, each trial constitutes a separate episode, and discrete time periods within each trial represent measurement occasions.

\item[Baseline Hazard] The pattern of hazard over time when all covariates equal zero (or their reference categories). Often modeled with time dummy variables or polynomials.

\item[Time-Varying Covariates (Time-Dependent Covariates)] Predictors whose values change over time periods within the same individual's episode (e.g., employment status, gaze position, age, income).

\item[Risk Set] For non-repeatable events with continuous time units: the set of individuals who are eligible to experience the event at a given time point. In discrete-time analysis, the risk set at time period t consists of all individuals who have not yet experienced the (non-repeatable) event by the beginning of period t and are still under observation. An individual exits the risk set once they experience the (non-repeatable) event or are censored. In trial-based or repeated measurements designs with discrete time, the risk set at each discrete time period within a trial consists of all observations (trials) where the event has not yet occurred by that time period, even when multiple trials come from the same individual.



\item[Non-Repeatable (Terminal) Events] Events that can occur only once for an individual and end the observation period (e.g., death, first marriage, onset of a chronic disease). Once the event occurs, the individual exits the risk set.

\item[Repeatable Events] Events that can potentially occur multiple times for the same individual (e.g., job changes, hospitalizations, pregnancies). The individual remains at risk for subsequent events after each occurrence. In analysis, these can be treated as independent events (analyzing only first occurrence or pooling all occurrences) or as recurrent events with interdependence.

\item[Recurrent Events] A specific type of repeatable event where the analysis explicitly models and accounts for the ordering and interdependence of multiple occurrences (first event, second event, third event, etc.). This approach recognizes that experiencing the event may change the risk of future occurrences.
...
\item[Discrete-time survival function] The probability that an individual survives (does not experience the event) beyond a particular time period t. Denoted as S(t) = P(T>t). Related to discrete-time hazard through S(t) = $\prod [1-h(j)]$ for all j$\leq$t.


\item[Right Censoring] Occurs when an individual's observation period ends before the event occurs (e.g., study ends, lost to follow-up). We know the event didn't occur by time t, but don't know if/when it occurs afterward.

\item[Left Censoring] Occurs when the event may have occurred before the observation period began, but we don't know exactly when.

\item[Interval Censoring] Occurs when we know the event happened within a specific time interval but we don't know the exact timing within that interval.

\item[Life Table] A tabular method for describing the distribution of event times, showing the number at risk, number of events, hazard, and survival probability for each time period.





\item[Person-Period Data Structure] A data format where each individual contributes multiple rows to the dataset per episode — one for each discrete time period they are at risk of experiencing the event.



\item[Time-Invariant Covariates] Predictors whose values remain constant throughout an individual's observation period (e.g., race, sex, birth cohort) or episode.




\item[Logit Model (Discrete-Time Hazard Model)] A common approach using logistic regression where the outcome is binary (event occurs vs. doesn't occur) in each person-period, and the log-odds of the event are modeled as a function of time and covariates.

\item[Complementary Log-Log Model (cloglog)]
An alternative to the logit model that has a direct correspondence to the proportional hazards assumption and continuous-time models. Often preferred when time intervals are short.

\item[Proportional Odds Assumption] In logit models, the assumption that the effect of a covariate multiplies the odds of event occurrence by a constant factor, regardless of time. Can be tested by including interactions between the covariate and time indicators.

\item[Proportional Hazards Assumption] The assumption that covariates multiply the baseline hazard by a constant factor, meaning the ratio of hazards between two individuals with different covariate values remains constant over time. Can be tested by including interactions between the covariate and time indicators.

\item[Competing Risks] Situations where multiple types of events can occur, and the occurrence of one event prevents the others from occurring (e.g., death prevents marriage; different types of job transitions).

\item[Unobserved Heterogeneity (Frailty)] Individual-level differences in event risk that are not captured by observed covariates. Can lead to spurious duration dependence if not addressed. Commonly modeled using hierarchical or multilevel models with random effects (also called frailty models), which account for unobserved individual-level variation in baseline risk.
...
<!--
Response time (RT) tasks can be categorized in several ways depending on what aspect you want to emphasize:
...
Choose correct response based on stimulus (e.g., left button for "A", right for "B")
Involves stimulus discrimination and response selection

**Go/No-Go**
Respond to target, withhold response to non-target
Tests response inhibition

**Stop-Signal**
Execute response unless stop signal appears
Measures ability to inhibit prepotent responses

## By Processing Demands

**Perceptual Tasks**
Stimulus detection (simple)
Discrimination (judging differences)
Identification (naming/categorizing)

**Memory Tasks**
Recognition (old/new judgments)
Recall with timed responses
Memory search (Sternberg task)

**Attention Tasks**
Visual search
Stroop task (interference/conflict)
...
## By Stimulus-Response Relationship

**Compatible**
Natural/intuitive mappings (move right joystick for rightward stimulus)

**Incompatible**
Arbitrary or conflicting mappings (Simon task, where location conflicts with response)

**Congruent vs. Incongruent**
Whether distractors match or mismatch the target

## By Number of Alternatives

**Binary choice** (2 options)
**Multiple choice** (3+ options)
Governed by Hick's Law: RT increases logarithmically with number of choices

## By Paradigm/Design

**Speeded vs. Accuracy-Emphasized**
Instructions emphasize speed vs. accuracy

**Blocked vs. Mixed**
Single condition blocks vs. mixed trial types

**Primed vs. Unprimed**
Whether preceding stimulus influences current trial

**Sequential Effects**
Examining how previous trial(s) affect current RT

## By Modality

**Input modality:**
Visual, auditory, tactile, multimodal

**Output modality:**
Manual (button press, keyboard)
Vocal
Eye movements (saccadic RT)
...
**Flanker** (distractor interference)
**Simon** (spatial compatibility)
**Mental rotation**
**Sternberg memory scanning**

The categorization you choose typically depends on your research question whether you're interested in decision processes, attention, inhibition, memory retrieval, etc.
...
Great question! Signal detection theory (SDT) paradigms occupy an interesting space in RT research. Here's how they fit:

## SDT Tasks as RT Paradigms

**Yes-No Detection/Discrimination**
"Is a signal present?" or "Is this stimulus A or B?"
Binary choice RT task
Participants make a detection or discrimination judgment
RT reflects the decision process plus response execution

**Rating Scale Tasks**
Multiple confidence levels (e.g., 1-6 scale)
Multiple-choice RT paradigm
RT increases with number of rating options (Hick's Law applies)

## Key Distinction: Accuracy vs. Speed Focus

Traditional SDT emphasizes:
**Accuracy** (hits, false alarms, d', criterion)
Often untimed or self-paced
Focus on sensitivity and bias

RT paradigms emphasize:
...
Unify RT distributions and accuracy
Evidence accumulation until threshold
Parameters map onto SDT concepts:
  Drift rate = sensitivity (d')
  Starting point  bias (criterion)
  Boundary separation = speed-accuracy tradeoff

**Sequential Sampling Models**
Treats decisions as accumulating noisy evidence over time
RT emerges naturally from when threshold is reached
Explains both correct and error RTs

## How to Categorize SDT-RT Tasks

**By stimulus characteristics:**
Detection (signal present/absent)
Discrimination (which of two signals)
Recognition (old/new memory)

**By decision type:**
Binary choice RT (yes-no, old-new)
Multiple choice RT (rating scales, multiple alternatives)

**By theoretical framework:**
Traditional SDT (accuracy-focused)
Sequential sampling (RT + accuracy)
Speed-accuracy tradeoff paradigms

## Practical Examples

**Yes-No Recognition Memory:**
Show word: old or new?
Measures both: accuracy (d'), RT for hits vs. correct rejections
RT often faster for high-confidence judgments

**Perceptual Discrimination:**
"Is tone A higher than tone B?"
Choice RT task with SDT analysis
Can measure both sensitivity and decision speed

**2AFC (Two-Alternative Forced Choice):**
"Which interval contained the signal?"
Choice RT + eliminates response bias issues from traditional SDT

## Why Combine RT + SDT?

1. **Richer data**: RT distributions reveal decision dynamics that accuracy alone misses
2. **Speed-accuracy tradeoffs**: Can't interpret accuracy without knowing time pressure
3. **Individual differences**: Some people are fast/sloppy, others slow/careful
4. **Process models**: Modern theories (DDM, LBA) predict both measures simultaneously

So SDT tasks are essentially **choice RT paradigms** where we also care deeply about the accuracy patterns (signal vs. noise trials), not just overall speed. They bridge perceptual/memory processes with decision-making dynamics.
...
Here's a comprehensive, updated list of SDT tasks organized by category:

## 1. Detection Tasks

**Visual Detection**
Detect dim light/spot on screen
Detect Gabor patch in noise
Detect motion in random dot kinematogram
Detect edge/contour in textured background
Detect target object in cluttered scene
Visual search (target present/absent)

**Auditory Detection**
Detect tone in white noise
Detect gap in continuous noise
Detect signal in background babble
Detect phoneme in speech stream

**Tactile Detection**
Detect vibration on skin
Detect pressure threshold
Detect temperature change

**Multimodal Detection**
Detect audiovisual synchrony
Cross-modal detection tasks

## 2. Discrimination Tasks

**Visual Discrimination**
Brightness discrimination (which is brighter?)
Orientation discrimination (tilted left vs. right?)
Spatial frequency discrimination
Size discrimination
Color discrimination
Face discrimination (same person or different?)
Emotion discrimination (happy vs. sad face?)

**Auditory Discrimination**
Pitch discrimination (higher vs. lower?)
Loudness discrimination
Temporal order discrimination
Phoneme discrimination (/ba/ vs. /pa/)
Music interval discrimination

**Other Modalities**
Texture discrimination (roughness)
...
Numerical comparison (which number is larger?)
  Symbolic (digits: 3 vs. 7)
  Non-symbolic (dot arrays)
Physical size comparison (which line is longer?)
Spatial comparison (which distance is farther?)
Angular comparison (which angle is greater?)

**Intensity Comparison**
Which tone is louder?
Which light is brighter?
Which stimulus is heavier/rougher/warmer?

**Temporal Comparison**
Which duration was longer?
Which event occurred more recently?
Which rhythm is faster?

**Cross-Modal Comparison**
Which is more intense: this light or this tone?

**Same-Different Tasks**
Simultaneous: Are these two stimuli the same or different?
Sequential: Is the second stimulus the same as the first?
Face matching, pattern matching, color matching
Delayed match-to-sample

**Relative Judgment**
Which is more pleasant/attractive/valuable?
...
**Verbal Memory**
Word recognition (old/new)
Sentence recognition
Story recognition
Source memory (which list/context?)

**Visual Memory**
Picture recognition (old/new)
Face recognition
Scene recognition
Object recognition
...
Paired associate recognition (were these paired at study?)
Face-name association
Item-context binding
Relational memory

**Temporal Memory**
Recency judgments (which was more recent?)
...
Category learning with novel stimuli
Prototype classification
Rule-based vs. similarity-based categories

**Semantic Categories**
Lexical decision (word vs. nonword)
Semantic categorization (food vs. tool)

**Perceptual Categories**
Classify along continuum (phonetic boundaries)
...
Simultaneity judgment (same time or different?)

**Duration Tasks**
Duration discrimination (which was longer?)
Duration categorization (short vs. long)

**Rhythm Tasks**
Regular vs. irregular rhythm detection
Beat detection

**Temporal Integration**
Detect signal integrated over time
Attentional blink (was target 2 present?)

## 7. Clinical and Applied Tasks

**Medical Diagnosis**
Disease present/absent from imaging
Radiological screening (tumor detection)
...
Security screening (threat detection at airports)
...
**Lie Detection**
Deception detection from behavior
Concealed information test
Credibility assessment

## 8. Social and Emotional Tasks

**Emotion Recognition**
Detect emotional expression (present/absent)
Discriminate emotions (angry vs. fearful)
Emotion intensity judgment

**Social Perception**
Detect deception/lying
Detect trustworthiness
Detect social threat
Detect sexual interest/attraction
Detect dominance/submission

**Theory of Mind**
Detect intentional vs. accidental actions
Discriminate beliefs/desires in others
False belief detection

**Face Processing**
Own-race vs. other-race face recognition
Familiar vs. unfamiliar faces
Emotion in faces
Age/gender classification

## 9. Metacognition Tasks

**Confidence Judgments**
Any task with confidence ratings (e.g., 6-point scale)
Post-decision wagering
Feeling of knowing judgments
Judgments of learning

**Type 2 SDT (Metacognitive Sensitivity)**
Discrimination of own correct vs. incorrect responses
...
**Speech Perception**
Phoneme detection/discrimination
Voice-onset time categorization
Speaker identification (same/different speaker)

**Reading and Lexical Processing**
Lexical decision (word/nonword)
Letter detection in words
Semantic relatedness judgments

**Comprehension**
Detect grammatical errors
Detect semantic anomalies
Text coherence judgments

## 11. Attention Tasks

**Spatial Attention**
Detect target at cued vs. uncued location
Visual search with set size manipulation

**Feature-Based Attention**
Detect target with specific feature
Conjunction search (target present/absent)

**Temporal Attention**
Attentional blink (T2 detection given T1)
Psychological refractory period tasks

**Divided Attention**
Dual-task detection/discrimination
Multiple target monitoring

## 12. Learning and Statistical Tasks

**Implicit Learning**
Detect learned patterns vs. random sequences
Statistical learning (detect regularities)
Artificial grammar learning (grammatical/ungrammatical)

**Associative Learning**
Detect contingency between cues and outcomes
Causal judgment (cause present/absent)

**Perceptual Learning**
Discrimination after training
Texture/expert object discrimination

## 13. Multi-Alternative Forced Choice

**2AFC (Two-Alternative Forced Choice)**
Which interval contained the signal? (temporal 2AFC)
Which location contained the signal? (spatial 2AFC)
Better/worse comparison

**3+ AFC**
Which of N intervals/locations contained the signal?
Multiple object identification
Multiple category choices

**Oddity Tasks**
Which of 3+ items is different/odd one out?

## 14. Change Detection

**Visual Change**
Did anything change between displays?
Change blindness paradigms
Flicker paradigm (detect what changed)

**Auditory Change**
Detect pitch/timbre change
Mismatch detection

**Cross-Modal Change**
Detect change across modalities

## 15. Complex/Multidimensional Tasks

**Multidimensional Discrimination**
Discriminate on multiple features simultaneously
General recognition theory (GRT) paradigms

**Hierarchical Detection**
Global vs. local level detection
Navon figures (large letter made of small letters)

**Conjunction/Feature Integration**
Detect conjunction of features
Feature binding tasks

## 16. Action and Motor Tasks

**Motor Detection**
Detect own movement vs. external movement
Agency detection (did I cause this?)

**Temporal Motor Tasks**
Synchronization detection (was action synchronized with event?)
Motor timing discrimination

## 17. Specialized Formats

**Adaptive/Staircase Procedures**
Any of above with adaptive threshold estimation
Quest, PEST, or simple up-down methods

**Rating Scale Variants**
4-point, 6-point, or continuous confidence ratings
Likelihood ratings (How likely is signal present?)
Unequal variance models

**Sequential/Dynamic Tasks**
Multiple observations before decision
Evidence accumulation over time
Dynamic noise conditions

**Context-Dependent Tasks**
Detection/discrimination with varying context
Expectation manipulation
Prior probability manipulation

## 18. Memory Strength Tasks

**Remember/Know Judgments**
Recognition with recollection quality
Familiarity vs. recollection

**Plurality Discrimination**
Was item seen once or multiple times?
Frequency discrimination

The key insight is that SDT can be applied to virtually any task where you're discriminating between two (or more) states, categories, or alternatives based on noisy internal representations. The common thread is the signal detection framework: comparing evidence against a criterion or comparing two distributions of evidence.
...
Moreover, as indicated by @singerAppliedLongitudinalData2003, learning discrete-time EHA methods first will help in learning continuous-time methods, so it seems like a good starting point.
...
The definitions of these and other discrete-time functions are given in section B of the Supplemental Material.
...
life_tab <- read_csv("../Tutorial_1_descriptive_stats/tables/lifetable_blank_s6.csv")
...
apa_table(
  ...
  placement = "H",
  format.args = list(
    digits = c(0,0,0,0,2,2,2,2,2,2,2,2),
...
(ref:descr-plot2-caption) Estimated sample-based discrete-time hazard (h), survivor (S), conditional accuracy (ca) and probability mass (P) functions for participant 6. Vertical dotted lines indicate the estimated median RTs. Error bars represent +/- 1 standard error of the respective proportion.
...
For example, for participant 6, the estimated hazard values in bin 280 are `r data_final[[7]][[6]] %>% filter(condition=="blank") %>% select(hazard) %>% slice(7) %>% pull()`, `r data_final[[7]][[6]] %>% filter(condition=="congruent") %>% select(hazard) %>% slice(7) %>% pull()`, and `r data_final[[7]][[6]] %>% filter(condition=="incongruent") %>% select(hazard) %>% slice(7) %>% pull()` for the blank, congruent, and incongruent prime conditions, respectively. In other words, when the waiting time has increased until *240 ms* after target onset, then the conditional probability of response occurrence in the next 40 ms is more than five times larger for both prime-present conditions, compared to the blank prime condition. 

Furthermore, the estimated conditional accuracy values in bin 280 are `r data_final[[7]][[6]] %>% filter(condition=="blank") %>% select(ca) %>% slice(7) %>% pull()`, `r data_final[[7]][[6]] %>% filter(condition=="congruent") %>% select(ca) %>% slice(7) %>% pull()`, and `r data_final[[7]][[6]] %>% filter(condition=="incongruent") %>% select(ca) %>% slice(7) %>% pull()` for the blank, congruent, and incongruent prime conditions, respectively. In other words, if a response is emitted in bin 280, then the probability that it is correct is estimated to be `r data_final[[7]][[6]] %>% filter(condition=="blank") %>% select(ca) %>% slice(7) %>% pull()`, `r data_final[[7]][[6]] %>% filter(condition=="congruent") %>% select(ca) %>% slice(7) %>% pull()`, and `r data_final[[7]][[6]] %>% filter(condition=="incongruent") %>% select(ca) %>% slice(7) %>% pull()` for the blank, congruent, and incongruent prime conditions, respectively.
...
ptb_data <- ptb_data %>% 
# select analysis time range: 200-600 ms, with 10 bins (time bin ranks 6 to 15)
filter(period > 5) %>%
...
Finally, because many researchers will be more familiar with frequentist statistics, we also provide code to fit hazard and conditional accuracy models in the frequentist framework in Tutorial_3a.Rmd and Tutorial_3b.Rmd, using the R package lme4() [@R-lme4]. 
...
All of the submitted manuscript and Supplemental Material was previously posted to a preprint archive: https://doi.org/10.31234/osf.io/57bh6_v2
...
Supplemental Material: Panis_Ramsey_suppl_material_R1.pdf
