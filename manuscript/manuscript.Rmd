...
As a simple illustration, Figure 1 summarises simulated data for one subject that shows how comparing means between two conditions conceals the shapes of the underlying RT and accuracy distributions. Indeed, compared to the aggregation of data across trials (Figure 1A), a distributional approach offers the possibility to reveal the temporal organization of psychological states (Figure 1B). Here we apply a distributional method known as **discrete-time event history analysis** (EHA) extended with **speed-accuracy tradeoff analysis** (SAT). All terms in bold can be found in the glossary (section 2.4). 

(ref:descr-fig1-caption) Simulated single-subject data showing mean performance versus a distributional analysis. (A) The mean RT (top) and overall accuracy (bottom) for two conditions are plotted. Two hundred trials are simulated in each condition. (B) The **discrete-time hazard functions** (top) and **conditional accuracy functions** (bottom) are plotted for the same data. The first second after target stimulus onset (time zero) is divided in ten time bins of 100 ms (indexed by t = 1 to 10). Hazard for bin t is the conditional probability of response occurrence in bin t given no events in earlier bins. Conditional accuracy for bin t is the conditional probability of a correct response given response occurrence in bin t. The hazard and conditional accuracy estimates are plotted at the upper bound of each time bin. The mathematical definitions of discrete-time hazard and conditional accuracy are provided in section X of the Supplemental Material. Error bars represent +/- 1 standard error of the mean (A) or proportion (B). 

```{r plot1, fig.cap = "(ref:descr-fig1-caption)", out.width="90%"}
knitr::include_graphics("../sims/figures/Fig1_revision1.jpeg")
```

For example, Figure 1B shows a first state (up to 400 ms after target onset) for which the early upswing in the hazard functions of response occurrence is equal for both conditions, and the emitted responses are always correct in condition 1 and always incorrect in condition 2. In a second state (400 to 500 ms), the hazard of response occurrence is higher in condition 1, and conditional accuracies are close to .5 in both conditions. In a third state (>500 ms), the effect disappears in hazard, and all conditional accuracies are equal to 1. Note that we will always refer to a time bin by its upper bound. For example, time bin "500" in Figure 1B refers to the time period running from 400 ms to 500 ms, with the lower bound of 400 ms excluded, and the upper bound of 500 ms included. Importantly from a face-validity perspective, this pattern of simulated data can be seen in the experimental psychology literature [@panisWhenDoesInhibition2022; @panisWhatShapingRT2016; @panisHowCanWe2020].
...
Our ultimate aim in this paper is twofold. First, we want to inform readers of the many benefits of using EHA when dealing with psychological RT data, and other types of time-to-event data. Second, we want to provide a set of practical tutorials, which provide step-by-step instructions on how you actually perform a (repeated event) discrete-time EHA on RT data, as well as a complementary discrete-time SAT analysis on timed accuracy data in case of choice RT data (Figure 1B).
...
<!--, compared to orthodox methods such as Analysis-of-variance, linear regression, and delta-plots (ref).-->
...
The fact that EHA can deal with right-censoring, therefore, presents a analytical strength of the approach compared to many common approaches in experimental psychology (e.g., ANOVA, linear regression, delta plots). Other types of censoring include **left censoring** and **interval censoring** (see also section A of the Supplemental Material).
...
There are several approaches to deal with a competing risks situation (see section X of the Supplemental Material). Here, we estimate and model the hazard of (any) response occurrence and extend it with a speed-accuracy trade-off (SAT) analysis (plotting and modeling accuracy as a function of latency; see Figure 1B).
...
EHA/SAT allows to answer several new research questions on behavioral response dynamics in experimental paradigms to study cognition:

- What is the shape of the hazard function of response occurrence? In other words, what is the effect of the passage of time on the within-trial (or across-bin) time scale on the hazard estimate? Does the hazard function increase to a plateau, or does it increase to a peak and then decrease? Is it flat in the right tail [@panisStudyingDynamicsVisual2020]?
...
- How and when do **time-varying predictors** (eye gaze position, eye dilation, heart rate, parietal-frontal synchronization, ...) affect the risk of event occurrence?
...
First, in contrast to orthodox approaches, EHA can deal with right-censored observations when estimating the hazard function of event occurrence. This is especially important in studies on visual masking, the attentional blink, the stop-signal task, etc.
...
\item[Risk Episode (Spell)] The duration from the start of observation or from a previous event until either the event occurs or observation ends. Also called a "spell" (particularly in labor economics). An episode represents one complete at-risk period being studied. Within each episode, data may be collected at multiple discrete time points (e.g., monthly or yearly intervals) — these are the time periods or measurement occasions. In discrete-time analysis, each episode is typically divided into multiple person-period observations, where each person-period is the actual unit of analysis for model estimation. In repeatable event contexts, individuals may contribute multiple episodes. For example, one person's marriage might be a single episode observed over 10 yearly measurements (resulting in 10 person-period observations), while in a trial-based experiment, one person completes 100 episodes (trials), each potentially lasting several time periods.
