...
Figure 1: plot hazard, S(t), F(t) and P(t) functions for 2 simulated conditions
...
Figure 2: Detection data +

### Repeated vs. recurrent events

In the context of response time experiments with button presses across trials, these would typically be considered repeated events rather than recurrent events in the survival analysis sense.
Here's why:
Key Differences
Recurrent events in survival analysis usually imply:

The event can "re-occur" after happening (like a disease relapse)
There's genuine risk of the same phenomenon happening again
Often involves a temporal process where earlier events may influence later ones
The focus is on when events happen over continuous time

Repeated measures (which button presses across trials represent):

Structured, often experimenter-determined observations
Each trial is typically independent or pseudo-independent by design
The focus is usually on the response time itself, not necessarily the temporal spacing between events
Trials are discrete, planned observations

For Response Time Data
Button presses across trials are better conceptualized as:

Repeated measures in a standard statistical sense
Each trial gives you a response time measurement
You're typically interested in modeling the distribution of response times and how they vary with conditions, not modeling the "survival" to a button press

Figure 4 :  perceptual dominance durations

### Competing risks: correct versus error responses

Figure 3: Discrimination data 
...
This distinction is why discrete-time hazard models can be fit using logistic regression - you're modeling a binary outcome (event yes/no) within each discrete time period.
 ...
 Figure 5: soa manipulation + old Figure 1
...
Natural for data collected at discrete intervals
