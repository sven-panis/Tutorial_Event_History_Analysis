...
```{r}
#glossary_path("../manuscript/glossary.yml")
...
#path <- tempfile("../manuscript/glossary_man", fileext = ".yml")
#glossary_path(path, create = TRUE)

#glossary_add("censoring", "A type of missing data problem.")
#glossary_add("event", "A qualitative change.")
...
We calculated the \textbf{hazard} for each time period. 
...
\item[Repeated Measurements (Trial-Based Events)] Events that are repeatedly measured within experimental trials or tasks, where each individual completes multiple independent trials and each trial yields one time-to-event observation (e.g., reaction time tasks, repeated decision-making experiments). Unlike recurrent events in natural history, these trials are typically designed to be independent realizations of the same process, though individual-level random effects may be needed to account for within-subject correlation.

\item[Continuous-Time Hazard (Hazard Rate, Instantaneous Hazard)] The instantaneous rate at which events occur at time point t, given survival to time t. Formally defined as the limit h(t) = lim($\Delta t \to 0$) P(t $\leq$ T < t+$\Delta t$ | T$\geq$t)/$\Delta t$. Unlike discrete-time hazard (a probability), continuous-time hazard is a rate that can exceed 1. Denoted as $\lambda(t)$.

\item[Risk Set] For non-repeatable events with continuous time: the set of individuals who are eligible to experience the event at a given time point. In discrete-time analysis, the risk set at time period t consists of all individuals who have not yet experienced the (non-repeatable) event by the beginning of period t and are still under observation. An individual exits the risk set once they experience the (non-repeatable) event or are censored. In trial-based or repeated measurements designs with discrete time, the risk set at each discrete time period within a trial consists of all observations (trials) where the event has not yet occurred by that time period, even when multiple trials come from the same individual.

\item[Survival Function] The probability that an individual survives (does not experience the event) beyond a particular time point. Denoted as S(t) = P(T>t). Related to hazard through S(t) = $\prod [1-h(j)]$ for all j$\geq$t.

\item[Hazard Function] The pattern of hazard probabilities over time, showing how the risk of event occurrence changes across time periods.

\item[Time Period (Interval)] The discrete unit of time used in the analysis (e.g., year, month, week). Each observation contributes one record per time period at risk.

\item[Episode (Spell, Risk Episode)] 
...
\item[Person-Period Data Structure] A data format where each individual contributes multiple rows to the dataset—one for each discrete time period they are at risk of experiencing the event.

\item[Time-Varying Covariates (Time-Dependent Covariates)] Predictors whose values change over time periods within the same individual's episode (e.g., employment status, age, income).

\item[Time-Invariant Covariates] Predictors whose values remain constant throughout an individual's observation period (e.g., race, sex, birth cohort).
...
\item[Interval Censoring] Occurs when we know the event happened within a specific time interval but don't know the exact timing within that interval.
...
In experimental psychology, it is standard practice to analyse response times (RTs), saccade latencies, and fixation durations by calculating average performance across a series of trials. Such comparisons between means have been the workhorse of experimental psychology over the last century, and have had a substantial impact on theory development as well as our understanding of the structure of cognition and brain function. Indeed, the view that mean values represent truth and variations around the mean are error is deeply ingrained in experimental psychology [@bolgerCausalProcessesPsychology2019]. However, differences in mean RT conceal important pieces of distributional information, such as when an experimental effect starts, how it evolves with increasing waiting time, and whether its onset is time-locked to other events [@panisHowCanWe2020; @panisNeuropsychologicalEvidenceTemporal2017; @panisStudyingDynamicsVisual2020; @panisTimecourseContingenciesPerceptual2009; @panisWhatShapingRT2016; @panisWhenDoesInhibition2022; @wolkersdorferTemporalDynamicsSequential2020]. 
...
```{r plot1-test, fig.cap = "(ref:descr-fig1-caption)", out.width="90%"}
knitr::include_graphics("../sims/figures/Fig1_revision1.jpeg")
```
-->
...
Our ultimate aim in this paper is twofold. First, we want to convince readers of the many benefits of using EHA when dealing with psychological RT data, and other types of time-to-event data. Second, we want to provide a set of practical tutorials, which provide step-by-step instructions on how you actually perform a (single event) discrete-time EHA.

<!--on RT data, as well as a complementary discrete-time SAT analysis on timed accuracy data in case of choice RT data (Figure 1B).-->
...
In what follows, the paper is organised in three main sections. 

In Section 2, we provide a brief overview of EHA to orient the reader to the basic concepts (events, censoring, hazard, competing risks), the various modeling approaches available, and the new types of research questions that EHA allows to answer about behavioral dynamics in experimental cognitive paradigms.
...
## 2.1 Basic concepts of EHA

### Repeated versus recurrent events

EHA is a class of statistical approaches to study the occurrence and timing of events, such as death, disease onset, marriages, arrests, and job terminations [@allisonSurvivalAnalysisUsing2010]. EHA is known by various labels, such as survival analysis, hazard analysis, duration analysis, failure-time analysis, and transition analysis [@singerAppliedLongitudinalData2003]. In this paper, we choose to use the term EHA throughout. An event is defined as a qualitative change -- a transition from one discrete state to another -- that can be situated in time. Typical events of interest in cognitive science include manual button presses, vocal response onsets, saccadic eye-movement onsets, fixation offsets, and spatial boundary crossings when measuring reach movement trajectories, for example in mouse-tracking studies (refs: mouse + KL omar). While some events can only happen once (e.g., death), most events of interest for cognitive scientists can happen repeatedly to the same individual.

To apply EHA one also must define time point zero (e.g., stimulus onset), and measure the passage of time between time point zero and event occurrence in discrete or continuous time units. In RT tasks, response times are typically measured with a temporal resolution of 1 millisecond, or higher. As 1 ms is typically short compared to the length of the observation period (e.g., 1 second), response times are generally treated as continuous time-to-event data.

To collect observations on the continuous variable RT, experimental psychologists typically administer a set of discrete, planned observations, a.k.a. trials, to each participant, and manipulate some aspect of the environment between trials. In Figure 1, we present a hypothetical single-subject RT data set from a detection experiment with one response button (i.e., a simple RT task) and two experimental conditions: target present and target absent. For example, each trial in each condition might start with a fixation screen for 1 s, a target display screen for 100 ms in which the target stimulus is presented or not depending on the experimental condition the trials belongs to, and finally a blank screen until an overt response is detected or 500 ms have passed. Figure 1A shows that the participant presses the button sometime before 500 ms in most trials in the target-present condition (black dots), while (s)he does not press the button in most trials of the target-absent condition (black crosses). In EHA parlance, the single event of interest is a button press response that is repeatedly measured, *time zero* is defined as target display onset, the  *observation or follow-up period* is 500 ms long in each trial or repeated measurement, in each trial the participant is *at risk* for response occurrence as long as the response has not occurred yet, and the individual starts in an "idle" state in each trial and *transitions* to a "detected" state when a response occurs.

(ref:descr-fig1-caption) Response time (RT) tasks and EHA concepts. The timer starts at time 0 (black vertical line) and stops at the censoring time (dotted vertical line). In each trial of each condition of each task, an overt response is indicated by a circle, and a right-censored observation by a cross. (A) Single-button detection data requires a single-event analysis. (B) Two-button discrimination data requires a competing risks analysis. Black (white) dots indicate correct (error) responses. (C) Competing risk data in combination with an stimulus-onset-asynchrony manipulation might require increasing the observation period in each trial. (D) Two-button bistable perception data requires a recurrent event / multistate analysis. Black bars indicate stimlus presentation.

<!--Simulated single-subject data showing mean performance versus a distributional analysis. (A) The mean RT (top) and overall accuracy (bottom) for two conditions are plotted. Two hundred trials are simulated in each condition. (B) The discrete-time hazard functions (top) and conditional accuracy functions (bottom) are plotted for the same data. The first second after target stimulus onset (time zero) is divided in ten time bins of 100 ms (indexed by t = 1 to 10). The hazard and conditional accuracy estimates are plotted at the upper bound of each time bin. The definitions of discrete-time hazard and conditional accuracy are further explained in section 2.1.2. Error bars represent +/- 1 standard error of the mean (A) or proportion (B). -->

```{r plot1-revision2-test, fig.cap = "(ref:descr-fig1-caption)", out.width="90%"}
knitr::include_graphics("../REVIEW2/Plot_RTtasks.png")
```

Recurrent events are events that happen repeatedly within the same observation period. For example, Figure 1D shows hypothetical data from a bistable perception task in which the participant is looking at an ambiguous stimulus (e.g., the duck-rabbit illusion, the Necker cube) for two minutes in each trial, and asked to press a button each time when her/his perception switches from one possibe interpretation to the other possible interpretation. In this task, there are two events (percept A switches to percept B, percept B switches to percept A) that can recur within the same observation period of two minutes, so that the individual transitions back and forth between two states. We do not analyse recurrent events in this tutorial. More information about recurrent events and multistate analysis can be found in @stoolmillerModelingHeterogeneitySocial2006, @millsIntroducingSurvivalEvent2011, and @lougheedMultilevelSurvivalAnalysis2019.

<!--These minimal requirements are fulfilled by the RT data obtained in single-button detection tasks, where the time-to-response is repeatedly measured in different trials in the same individual. In section A of the Supplemental Material we visualize this and other types of time-to-event data which are typically obtained in discrimination and bistable perception tasks.-->

### Types of censoring

By convention, experimental psychologists either (a) use a response deadline and discard all trials without a response, or (b) wait in each trial until a response occurs and then apply data trimming techniques, i.e., discarding too long RTs (and perhaps also erroneous responses and too short RTs), before calculating a mean RT [@bergerComparisonDifferentResponse2021]. 
Discarding data can introduce biases, however.
EHA treats such trials as *right-censored* observations on the variable RT, because all we know is that RT is greater than some value. 
Right censoring is a type of missing data problem and a nearly universal feature of survival data including RT data [@allisonSurvivalAnalysisUsing2010]. For example, all trials in Figure 1A have a censoring time of 500 ms, and some trials result in observed event times (those with a RT below 500 ms), while other trials result in response times that are right-censored at 500 ms. The fact that EHA can deal with right censoring presents a analytical strength of the approach compared to many common approaches in experimental psychology (e.g., ANOVA, linear regression, delta plots).
...
### Describe the shape of a distribution of time-to-event data

Because our Tutorials focus on discrete-time methods as explained in section 2.3, we assume now that the continuous-time RTs have been discretized by partioning the observation period in each trial into bins of 20 ms, indexed by t (= 1, 2, ...). In particular, suppose (a) we measure a sample of continous-time RTs in the target-present condition of Figure 1A using a censoring time of 1200 ms, (b) that the occurrence and timing of responses is governed either by a normal or ex-gaussian distribution, and (c) that we apply interval censoring using intervals of 20 ms. As shown in Figure 2, the resulting distribution of interval-censored event times can be described in various ways, the first two of which are highly familiar to cognitive scientists.
...
- What is the shape of the hazard function? In other words, what is the effect of the passage of time on the within-trial (or across-bin) time scale on the hazard estimate? Does the hazard function increase to a plateau, or does it increase to a peak and then decrease? Is it flat in the right tail?

- Does an experimental manipulation affect the early or late part of the distribution? Do experimental effects increase or decrease in strength over time, or are they restricted only to a short time period within a trial?
...
- Is the shape of the hazard function within a trial changing as time passes by on longer time scales? For example, some effects might increase, or decrease in size and disappear, over the course of an experiment (across-trial time scale).

- Do time scales interact? For example, people typically speed up during the course of an experiment, but is this reflected in hazard changes in the left tail only (as reflected in a interaction between time bin and trial number)?
...
- Are experimental effects time-locked to certain stimuli?

- Are there inter-individual differences in the shape of the hazard function?
...
- Do computational models predict the shape of the hazard function as well as the mean RT?

- When does the effect of a spatio-temporally timed TMS pulse happen?
