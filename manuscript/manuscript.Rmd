...
```{r plot1-revision2, fig.cap = "(ref:descr-fig1-caption)", out.width="90%"}
...
The discrete-time hazard function or h(t) = Prob(RT = t | RT >= t) displays for each bin the conditional probability that the response occurs in bin t given that the response does not occur in earlier bins (t-1, t-2, ..., t=1). It is also known as the conditional probability mass function because h(t) = P(t)/S(t-1).

EHA focuses on describing and modeling the hazard function of event occurrence because (a) dynamic info ..., (b) diagnostic..., especially in the right tail.

(ref:descr-fig2-caption) In discrete time, the shape of a RT distribution can be described using the probability mass function (A), the cumulative distribution function (B), the survivor function (C), and the discrete-time hazard function (D).

```{r plot2-revision2, fig.cap = "(ref:descr-fig2-caption)", out.width="90%"}
...
```{r plot3-revision2, fig.cap = "(ref:descr-fig3-caption)", out.width="90%"}
...
(ref:descr-fig4-caption) Theoretical waiting time distributions.
...
This continous-time modeling approach divides the obseration period into bins, and assumes that hazard is exponentially distributed in each bin. It is great when ...

* Discrete-time hazard model

Discrete-time hazard models can be used when data is collected at discrete intervals. For example, when participants are tested at noon on each of six subsequent days for their memory of a set of pictures, the time-to-forgetting data are measured in discrete time units (e.g., picture A is forgotten on day 3, picture B on day 5).
However, they can also be used for data collected in continuous time units, when the hazard function is unknown and the analyst is interested in studying the shape of the hazard function and the effects of predictor variables.

Figure 5 presents empirical hazard and conditional accuracy functions from three popular RT tasks: visual search, spatial cueing, and the stop-signal task. 

(ref:descr-fig5-caption) Empirical distributions.

```{r plot5-revision2, fig.cap = "(ref:descr-fig5-caption)", out.width="90%"}
knitr::include_graphics("../REVIEW2/Plot_distributions_empirical.png")
```

...modeling a binary outcome (event occurred yes or no) within each discrete time period or bin or interval.
This distinction is why discrete-time hazard models can be fit using logistic regression  you're modeling a binary outcome (event yes/no) within each discrete time period.
...
Key Advantages of Discrete-Time Models
Easy to incorporate time-varying covariates
Flexible baseline hazard specification
Can use standard logistic regression software

Can handle ties easily
...
What is the shape of the hazard function? Does it increase to a plateau, or does it increase to a peak and then decrease?

Does an experimental manipulation affect the early or late part of the distribution?

Do experimental effects increase or decrease over time, or are they restricted only to a short time period?

Is the shape of the hazard function within a trial changing as time passes by on longer time scales?

Do time scales interact? Do predictor variables interact? Do time scales interact with predictor variables?
