...
We first provide a brief overview of hazard analysis to orient the reader to the basic concepts that we will use throughout the paper. However, this will remain relatively short, and for detailed treatment, see @singerAppliedLongitudinalData2003, @allisonDiscreteTimeMethodsAnalysis1982, and @allisonSurvivalAnalysisUsing2010. 
...
## Model 1: A general specification of TIME, and main effects of congruency and trial number
...
## Model 2: A polynomial specification of TIME, and main effects of congruency and trial number
...
## Model 3: A polynomial specification of TIME, and relaxing the proportionality assumption
...
## Model 4: A polynomial specification of TIME, and relaxing all three assumptions
...
## Plot congruency effects and subject-specific fits for model M4.
...
The hazard models in this tutorial assume that there is one event of interest. For RT data, this event consitutes a single transition between an "idle" state and a "responded" state. However, in certain situations, more than one event of interest might exist. For example, an individual might transition back and forth between a "healthy" state and a "depression" state, before being absorbed in a final "death" state. When you have data on the timing of these transitions, one can apply multi-state models which generalize survival analysis to transitions between three or more states [@SteeleF2004].
