...
In terms of minimum requirements to apply a single-event EHA, one must be able to:
...
There are different types of EHA.
...
Thus, by trading a bit of temporal resolution for a lower number of trials, discrete-time methods seem ideal for dealing with typical psychological RT data sets for which there are less than ~200 trials per condition per experiment [@panisAnalyzingResponseTimes2020].
...
We do not cover these benefits in detail here, as these are more general topics that have been covered elsewhere in textbooks (see also section F of the Supplemental Material). 
...
For instance, in Figure 1B we chose 100ms in an arbitrary manner.
...
Additionally, to further simplify the process for other users, the first two tutorials rely on a set of our own custom functions that make sub-processes easier to automate, such as data wrangling and plotting functions (see section C in the Supplemental Material for a list of the custom functions).
...
Next, we can set up the life tables and plots of the discrete-time functions h(t), S(t), ca(t), and P(t). To do so using a functional programming approach, one has to nest the data within participants using the group_nest() function, and supply a user-defined censoring time and bin width to our custom function "censor()", as follows.
...
In general, it is important to visually inspect the functions first for each participant, in order to identify individuals that may be guessing (e.g., a flat conditional accuracy function at .5 indicates that someone is just guessing), outlying individuals, and/or different groups with qualitatively different behavior. Also, to select a bin width for fitting models, one should test and compare various bin widths in the censor function, and select the smallest one that is supported by the data. Too small bin widths will result in erratic hazard functions because many bins will have estimates equal to zero.
...
## 3.3 Tutorial 2a: Fitting Bayesian hazard models to discrete time-to-event data
...
From a theoretical perspective, adding more than one time scale -- and their interactions -- can be important to capture plasticity and other learning effects that may play out on such longer time scales, and that are probably present in each experiment in general (REF DFT).
