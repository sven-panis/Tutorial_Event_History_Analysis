...
wordcount         : "10131 (body) + 1709 (references) + 3473 ( body supplemental material) + 393 (refs suppl. mat.)"
...
However, continuous forms of EHA require much more data to estimate the continuous-time hazard (rate) function well [@bloxomEstimatingResponseTime1984; @luceResponseTimesTheir1991; @vanzandtHowFitResponse2000]. 
...
To make causal inferences from the parameter estimates in model M1i, we first plot the densities of the draws from the posterior distributions of its population-level parameters in Figure 3, together with point (median) and interval estimates (80% and 95% credible intervals). A credible interval is a range of values that contains a parameter's true value with a specified probability, given the observed data and model.

(ref:descr-plot3-caption) Medians and 80/95% credible intervals of the posterior distributions of the population-level parameters of model M1i.

```{r plot-fixed-effects, fig.cap = "(ref:descr-plot3-caption)", out.width='80%'}
...
The point estimates and quantile intervals can also be reported in a table (see Tutorial_2a.Rmd for details).
...
To make inferences from the parameter estimates in model M1i_ca, we first plot the densities of the draws from the posterior distributions of its population-level parameters in Figure 6, together with point (median) and interval estimates (80% and 95% credible intervals).

(ref:descr-plot6-caption) Medians and 80/95% credible intervals of the posterior distributions of the population-level parameters of model M1i_ca. ca = conditional accuracy.
