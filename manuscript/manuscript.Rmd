...
wordcount         : "11101 (body) + 1807 (references) + 5328 (body supplemental material) + 589 (refs suppl. mat.)"
...
Also, because continuous forms of EHA require a lot of data to reliably estimate the shape of the **continuous-time hazard** (rate) function [@bloxomEstimatingResponseTime1984; @luceResponseTimesTheir1991; @vanzandtHowFitResponse2000], discrete-time methods seem ideal for dealing with typical psychological RT data sets for which there are typically less than ~200 trials per condition per participant.
...
In other words, as the waiting time in a trial is increasing incrementally in small steps, hazard reflects the risk that the event occurs in the current bin t, given that it has not yet
...
\item[Risk Episode (Spell)] The duration from the start of observation or from a previous event until either the event occurs or observation ends. Also called a "spell" (particularly in labor economics). An episode represents one complete at-risk period being studied. Within each episode, data may be collected at multiple discrete time points (e.g., monthly or yearly intervals) — these are the time periods or measurement occasions. In discrete-time analysis, each episode is typically divided into multiple person-period observations, where each person-period is the actual unit of analysis for model estimation. In repeatable event contexts, individuals may contribute multiple episodes.
