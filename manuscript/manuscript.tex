...
Set within this context, our overall aim is to introduce a set of tutorials, which explain \emph{how} to do such analyses in the context of experimental psychology, rather than repeat in any detail \emph{why} you may do them (but see section 2.1.3 and section I of the Supplemental Material). Therefore, we hope that our tutorials will provide a pathway for research avenues in experimental psychology that have the potential to benefit from using EHA in the future.
...
In this section, we want to provide an intuition regarding how EHA works in general, as well as in the context of experimental psychology. We assume readers are familiar with R and (logistic) regression equations. For those who want more detailed treatment of regression equations, we refer the reader to several excellent textbooks (Gelman, Hill, \& Vehtari, 2020; Winter, 2019). We decided to keep this text intentionally equation-light, and provide additional information and mathematical definitions in the Supplemental Material, including relevant regression equations (see section G of the Supplemental Material).
...
There are several approaches to deal with a competing risks situation (see section A of the Supplemental Material). Here, we estimate and model the \textbf{hazard} of (any) response occurrence and extend it with a SAT analysis (plotting and modeling accuracy as a function of latency; see Figure 1B).

\subsubsection{2.1.2 Modeling approaches}\label{modeling-approaches}

There are several different modeling approaches available between which one can select when analyzing a time-to-event data set. The type of models employed, and the definition of hazard, depend on whether one is using continuous or discrete time units. Even though RT is typically treated as a continuous variable, here we focus on discrete-time methods that can be applied after grouping RTs into time periods (or intervals, bins). Discrete-time methods are particularly useful when (a) the shape of the hazard function is unknown and the analyst wants to estimate it flexibly without parametric assumptions, and (b) the analyst is interested in examining and interpreting the shape of the \textbf{baseline hazard function}. In section D of the Supplementary Material we provide some examples to justify our choice of discrete-time modeling.
Also, because continuous forms of EHA require a lot of data to reliably estimate the shape of the \textbf{continuous-time hazard} (rate) function (Bloxom, 1984; Luce, 1991; Van Zandt, 2000), discrete-time methods seem ideal for dealing with typical psychological RT data sets for which there are less than \textasciitilde200 trials per condition per participant as they allow a tradeoff between temporal resolution (i.e., bin width) and the number of repeated measurements per condition per participant (Panis, Schmidt, et al., 2020).
...
\begin{itemize}
\item
  What is the shape of the hazard function of response occurrence? In other words, what is the effect of the passage of time on the \emph{within-trial} (or across-bin) \emph{time scale} on the hazard estimate? Does the hazard function increase to a plateau, or does it increase to a peak and then decrease? Is it flat in the right tail (Panis, Moran, et al., 2020)?
\item
...
\subsection{2.4 Glossary}\label{glossary}
