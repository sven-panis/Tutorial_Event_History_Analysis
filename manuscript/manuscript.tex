...
EHA is a class of statistical approaches to study the occurrence and timing of events, such as death, disease onset, marriages, arrests, and job terminations (Allison, 2010). EHA is known by various labels, such as survival analysis, hazard analysis, duration analysis, failure-time analysis, and transition analysis (Singer \& Willett, 2003). In this paper, we choose to use the term EHA throughout. An \textbf{event} is defined as a qualitative change -- a transition from one discrete state to another -- that can be situated in time. Typical events of interest in cognitive science include manual button presses, vocal response onsets, saccadic eye-movement onsets, fixation offsets, and spatial boundary crossings when measuring reach movement trajectories, for example in mouse-tracking studies (Schoemann, O'Hora, Dale, \& Scherbaum, 2021; Spivey \& Dale, 2006). While some events can only happen once (e.g., death, onset chronic disease), most events of interest for cognitive scientists can happen repeatedly to the same individual.
...
bin t, given that the event \emph{does not occur in previous bins}.
