...
\CommentTok{\#ptb\_data \textless{}{-} read\_csv("Tutorial\_1\_descriptive\_stats/data/inputfile\_hazard\_modeling.csv")}
\CommentTok{\# select analysis time range: (200,600] with 10 bins (time bin ranks t=6 to t=15) of 40 ms}
\CommentTok{\#ptb\_data \textless{}{-} ptb\_data \%\textgreater{}\% filter(period \textgreater{} 5)}
\CommentTok{\# create factor condition, with "blank" as the reference level}
\CommentTok{\#ptb\_data \textless{}{-} ptb\_data \%\textgreater{}\% mutate(condition = factor(condition, labels = c("blank", "congruent","incongruent")))}
\CommentTok{\# center discrete TIME (period) on bin 9, and trial on trial 1000}
\CommentTok{\#ptb\_data \textless{}{-} ptb\_data \%\textgreater{}\% mutate(period\_9 = period {-} 9,}
  \CommentTok{\#                              trial\_c = (trial {-} 1000)/1000)}
\CommentTok{\# remove unnecessary columns before fitting a model}
\CommentTok{\#ptb\_data \textless{}{-} ptb\_data \%\textgreater{}\% select({-}c(bl,tr,trial,period)) \# 12840 obs. of 5 variables}
\end{Highlighting}
\end{Shaded}
...
Another limitation of multilevel hazard modeling is that it might take some time to fit the model.
