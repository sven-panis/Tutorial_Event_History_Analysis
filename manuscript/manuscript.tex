...
The discrete-time hazard function or h(t) = Prob(RT = t \textbar{} RT \textgreater= t) displays for each bin the conditional probability that the response occurs in bin t given that the response does not occur in earlier bins (t-1, t-2, \ldots, t=1). It is also known as the conditional probability mass function because h(t) = P(t)/S(t-1).

EHA focuses on describing and modeling the hazard function of event occurrence because (a) dynamic info \ldots, (b) diagnostic\ldots, especially in the right tail.
...
\caption{In discrete time, the shape of a RT distribution can be described using the probability mass function (A), the cumulative distribution function (B), the survivor function (C), and the discrete-time hazard function (D).}\label{fig:plot2-revision2}
\end{figure}
...
\caption{Theoretical waiting time distributions.}\label{fig:plot4-revision2}
\end{figure}
...
This continous-time modeling approach divides the obseration period into bins, and assumes that hazard is exponentially distributed in each bin. It is great when \ldots{}
...
\caption{Empirical distributions.}\label{fig:plot5-revision2}
\end{figure}

\ldots modeling a binary outcome (event occurred yes or no) within each discrete time period or bin or interval.
