...
Tutorials 3a and 3b show how to implement multilevel models for hazard and conditional accuracy in the frequentist framework, respectively.
...
\noindent F(t) = 1-S(t) = P(RT \(\leq\) t). \hfill  (4)

The discrete-time hazard probability gives you the probability that the event occurs (sometime) in bin t, given that the event has not occurred yet in previous bins. Unlike the discrete-time hazard function, which assesses the unique risk of event occurrencce associated with each time bin, the discrete-time survivor function cumulates the bin-by-bin risks of event \emph{non}occurrence to obtain the probability that the event occurs afer bin t.
...
which gives you the probability that a response is correct given that it has been emitted in time bin t (Allison, 2010; Kantowitz \& Pachella, 2021; Wickelgren, 1977). This latter function is also known as the micro-level speed-accuracy tradeoff function.
...
\item
  calc\_ca\_2IV(df)
\item
  join\_lt\_ca(df1,df2)
\item
  join\_lt\_ca\_2IV(df1,df2)
\item
  extract\_median(df)
\item
  extract\_median\_2IV(df)
\item
  plot\_eha(df,subj,haz\_yaxis)
\item
  plot\_eha\_2IV(df,subj,haz\_yaxis)
\item
  plot\_eha\_agg(df,subj,haz\_yaxis)
\end{itemize}
