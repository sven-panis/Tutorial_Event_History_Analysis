...
\subsubsection{2.1.1 EHA concepts applied to RT tasks / Terminology and minimum requirements for EHA}\label{eha-concepts-applied-to-rt-tasks-terminology-and-minimum-requirements-for-eha}
...
For such \textbf{repeated measurements} or trial-based events, the \textbf{observation period} refers to the entire experimental session during which multiple independent trials (or \textbf{episodes}) are completed. In each trial, the individual is ``at risk'' for event occurrence as long as the participant is eligible to experience the outcome event at a given time point. In other words, a participant is considered ``at risk'' when (a) they have not yet experienced the event of interest in a trial, and (b) they are still under observation.
...
There are several different modeling approaches available between which one can select when analyzing a time-to-event data set. The type of models employed, and the definition of hazard, depend on whether one is using continuous or discrete time units. Even though RT is typically treated as a continuous variable, here we focus on discrete-time methods that can be applied after grouping RTs into time periods (or intervals, bins). Discrete-time methods are particularly useful when (a) the shape of the hazard function is unknown and the analyst wants to estimate it flexibly without parametric assumptions, and (b) the analyst is interested in examining and interpreting the shape of the \textbf{baseline hazard function}. In section X of the Supplementary Material we provide some examples to justify our choice of discrete-time modeling.
...
\item
  Do \textbf{time-varying predictors} (eye gaze position, eye dilation, heart rate, parietal-frontal synchronization, \ldots) affect the risk of event occurrence?
\end{itemize}
...
\textbf{Right-censoring} is a type of missing data problem and a nearly universal feature of survival data including RT data.
