...
\caption{In discrete time, the shape of a empirical RT distribution can be described using the probability mass function (A), the cumulative distribution function (B), the survivor function (C), and the discrete-time hazard function (D). Samples are drawn from three example distributions (two normal and one exgaussian distribution). Note the small differences in P(t), F(t), and S(t) and the relatively large difference in both hazard estimates around 800 ms. Sample-based hazard will hit 1 in the bin with the largest observed response time when none of the observations are right-censored. Also, there is always increased uncertainty around hazard estimates in later time bins.}\label{fig:suppl-fig2}
\end{figure}
...
\caption{Decision Tree.}\label{fig:suppl-fig4}
\end{figure}
...
Cox regression models (ref) assume time is measure in continuous time units and can be used when the analyst has no interest in the shape of the hazard function and only wants to test how hazard is affected by predictor variables.
...
This continous-time modeling approach divides the follow-up period into discrete bins, and assumes that continuous-time hazard is exponentially distributed in each bin.
