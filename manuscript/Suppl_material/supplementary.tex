...
\emph{Recurrent events} are events that happen repeatedly within the same follow-up period. For example, Supplementary Figure 1D shows hypothetical data from a bistable perception task in which the participant is looking at an ambiguous stimulus (e.g., the duck-rabbit illusion, the Necker cube) for two minutes in each trial (the follow-up period), and asked to press a button each time when her/his perception switches from one possible interpretation to the other possible interpretation. In this task, there are two events (percept A switches to percept B, percept B switches to percept A) that can recur within the same observation period of two minutes, so that the individual transitions back and forth between two states. Thus, each individual contributes multiple, sequential risk episodes within the follow-up period in each trial. We do not analyse recurrent events in this tutorial. More information about recurrent events and multistate analysis can be found in Stoolmiller and Snyder (2006), Mills (2011), and Lougheed, Benson, Cole, and Ram (2019).
...
For each of three example distributions (two normal and one exgaussian distribution) a large random sample was taken, and the RTs are grouped into 20 ms intervals.
...
\noindent P(t) = P(RT = t) = h(t).S(t-1) \hfill  (1)
...
\noindent F(t) = P(RT \(\leq\) t) = 1-S(t) \hfill  (2)
...
\noindent S(t) = P(RT \(>\) t) = 1-F(t) \hfill  (3)
...
\noindent h(t) = P(RT = t\textbar{} RT \(\geq\) t) = P(t)/S(t-1) \hfill  (4)
...
Parametric hazard models assume time is measured in continuous units and can be used when the shape of the hazard function is known. For example, Supplementary Figure 5 displays six members for each of four example distribution families.
...
For example, based on Supplementary Figure 5C and 5D, this participant transitions through at least three motor states during a trial: a first state running from -160 to -40 ms, reflecting the automatic response to the peripheral cue; a second state running from 0 to 160 ms, where conditional accuracies reverse (due to selective suppression of a premature response tendency); and a third state where conditional accuracy increases to 1 and the hazard functions display the inhibition-of-return effect (i.e., higher hazard for invalid compared to valid cueing).
...
These examples illustrate why we advocate the use of discrete-time methods in RT research as the entry method to EHA/SAT: they allow us to learn about the shape of the hazard function in a cognitive task when we do not know its shape. Other advantages of discrete-time hazard modeling include (a) it is easy to incorporate time-varying covariates, (b) flexible baseline hazard specification, (c) one can use logistic regression as you're modeling a binary outcome (event yes/no) within each discrete time bin. Next to the logit link function, the complementary log-log link is available when events can in principle occur at any time point within a bin, which is the case for RT data.
