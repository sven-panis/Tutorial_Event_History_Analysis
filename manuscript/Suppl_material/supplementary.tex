...
The shape of a distribution of waiting times can be described in multiple ways (Luce, 1991).
...
Supplementary Figure 2 shows four ways to describe the shape of a distribution of response times.
For each of three example distributions (two normal and one exgaussian distribution) a large random sample was taken, and the RTs are grouped into 20 ms intervals. In other words, the continuous-time RTs are interval censored. Now, let RT be a discrete random variable denoting the rank of the time bin in which a particular person's response occurs in a particular trial.
The resulting distribution of interval-censored event times can be described in various ways, the first two of which are highly familiar to cognitive scientists.
...
\caption{Empirical distributions observed in cognitive RT tasks. Sample-based hazard (A, C, E) and conditional accuracy functions (B, D, F). (A,B). Data from Panis et al.~(2020). (C,D). Data from Panis and Schmidt (2022). (E.F.) Unpublished data.}\label{fig:suppl-fig6}
\end{figure}

These empirical examples illustrate why we advocate the use of discrete-time methods in RT research as the entry method to EHA/SAT: The shape of empirical hazard functions often do not resemble those of popular parametric distributions. In other words, for many cognitive paradigms the shape of the hazard function is unknown, and discrete-time methods allow the analyst to learn about the shape of the hazard function, which can constrain interpretation.
...
Popular link functions in discrete-time hazard modeling include the logit link and the complementary log-log link, as shown in Supplementary Figure 2.
