...
When there are multiple events of interest, such as death from accident vs. natural causes, or correct versus error responses, there is a *competing risks* situation. For example, Supplementary Figure 1B displays a hypothetical single-subject RT data set from a discrimination experiment with two response buttons and two experimental conditions: A and B. Each trial in each condition might start, for example, with a fixation screen for 1 s, followed by a target display screen for 100 ms in which a female or male face is presented, and finally a blank screen until an overt response is detected or 800 ms have passed since face onset. Participants are required to press the left button when identifying a male, and the right button when identifying a female. 
...
\noindent h(t) = P(RT = t| RT $\geq$ t) = P(t)/S(t-1)   \hfill  (4)
