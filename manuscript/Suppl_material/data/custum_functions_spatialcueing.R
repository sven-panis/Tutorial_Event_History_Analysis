...
plot_eha_2IV <- function(df,subj,haz_yaxis=1,first_bin_shown=1,aggregated_data=F,Nsubj=6){
  ...
  data_medians <- tibble(period= data_median,
                         survival = rep(c(.5, 0),n_conditions),
                         cue =  factor(rep(rep(1:n_iv2, each=2), n_iv1),
                                         levels=c(1:3),
                                         labels=c("valid","invalid","no cue")),
                         soa = factor(rep(1:n_iv1, each=2*n_iv2),
                                       levels=c(1:4),
  ...
  # plot the hazard functions
  p1 <-df %>% 
    mutate(soa = condition1,       # Create correctly named factor (here: mask)
           cue = condition2) %>%  # Create correctly named factor (here: prime)
    ggplot(aes(x=period, color=cue, group=cue)) + # Apply correctly named factor
    ...
    theme(panel.grid.minor = element_blank(),
          axis.text.x = element_text(angle=90)) +
    facet_wrap(~soa, nrow=1, ncol=4) # Apply correctly named factor
  
  # plot the survivor functions
  p2 <-df %>%
    mutate(soa = condition1,      # Create correctly named factor
           cue = condition2) %>% # Create correctly named factor
    ggplot(aes(x=period, color=cue, group=cue)) + # Apply correctly named factor
