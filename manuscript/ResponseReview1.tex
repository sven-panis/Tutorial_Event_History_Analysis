...
Methods and Practices in Psychological Science (AMPPS). I have now
received input from three experts in the field, and you will find the
reviews below this message and attached. As you will see, all three
reviewers believe your paper can make a strong contribution to the
literature. Prior to considering the reviewers' input, I read your paper
to form an independent opinion. I share the positive sentiments about
this work and invite you to revise and resubmit your manuscript for
further consideration at AMPPS.
\end{quote}
...
\begin{quote}
To submit your revision, log into
...
revised manuscript. Please delete any redundant or outdated files before
completing the submission.
\end{quote}

\begin{quote}
Once again, thank you for submitting your manuscript to AMPPS and I look
...
\begin{quote}
David A. Sbarra, Ph.D. Editor, Advances in Methods and Practices in
...
Response: In the revised ms. We added the following section: In this
tutorial we provide code to implement discrete-time hazard models in a
Bayesian framework, with a focus on cognitive experimental psychology.
...
Response: On page x of the revised ms., we mention that we estimate
single-event occurrences that are repeatedly measured in each
participant.
...
Response: We disambiguate the various term early in the revised ms.
(page x).
...
Response: We address the issue of treating time as discrete or
continuous on page x of the revised ms.
...
Response: We removed repetitive text, and try to focus on clarity and
brevity in the revised ms.
...
Response: We discuss the issue of defining bins on page x of the revised
ms.
...
Response: We include a short comparison of both methods on page x of the
revised ms.
...
Response: We explain model weights on page x, include a reference for
dWAIC, and report the weights with 3 digits.
...
Response: We removed the frequentist examples from the main text into
...
works on page x of the revised ms. We address the issue of treating time
as discrete or continuous on page x of the revised ms. We removed
redundancies and included more precise explanations of technical jargon.
...
Response: We explain the location of the files in the README file. We
included a brief description of a life table and how to interpret the
plot in the R Mardown tutorial files. A single download step is possible
by either cloning the project, or downloading the ZIP file.
...
Response: This is a tutorial specifically focused on Bayesian methods
and cognitive experimental psychology data, such as response times and
saccade latencies. Analyzing such event latencies with EHA can give
better answers about the time course of psychological processes compared
to (a) ANOVA on means and (b) currently popular quantile-based
distributional methods. Information about the time course of cognitive
processes is present in the detailed shape of the hazard function which
quantifies the moment-to-moment changes in behavioral performance, in
terms of the risk of response occurrence, and the accuracy of responses
emitted. EHA thus allows answering research questions about the onset of
psychological effects, about time-locking of effects to stimuli, and
about how the effect changes on multiple time scales, i.e., with the
passage of time within trials, across trials, across blocks within a
session, etc. Applying EHA also has implications for experimental
design, such as response deadlines, timer on trial onset, etc.
...
Response: We believe a stand-alone tutorial for cognitive experimental
psychology is necessary because (a) psychologists are reluctant to
abandon ANOVA and switch to EHA/SAT for several reasons, (b) as far as
we know, none of the existing tutorials focuses on Bayesian methods.
...
We also made major cuts to both the introduction and the discussion
sections.
...
Response: We provide clear descriptions of censoring and risk sets on
page x of the revised ms.
...
Response: We placed the equations for h(t), S(t), and ca(t) in the
revised ms. on page x. The equations for P(t) are still in the
...
Response: We included the following text on page x of the revised ms.:
This tutorial deals with single-event analyses which focus on the time
until a single event occurs in a trial (e.g., a button press), with
repeated measures obtained for each individual. Recurrent event analysis
deals with multiple events that can happen repeatedly to the same
individual within a single observation period.
...
Response: We follow the suggestion of this reviewer to start the
tutorial at section 2.3. That is, we immediately provide an example
experiment/data set with a descriptive analysis that illustrates the
benefits of applying EHA to psychological time-to-event data such as
response times and saccade latencies. This new example data set is taken
from Panis \& Schmidt (2022).
...
place (page x of the revised ms.).
...
Response: We unpack in more detail how individuals can be identified
that may be guessing, or that do not perform the task as intended.
...
Response: We provide a brief description of the rationale of comparing
models M0i and M1i on page x of the revised ms.
...
about how long a psychological state lasts within a trial.
