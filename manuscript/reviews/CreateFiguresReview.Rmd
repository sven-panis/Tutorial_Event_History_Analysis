...
# 2. Create Supplementary Figure 1 on experimental paradigms
...
create bistable perception plot
...
p7 <- data_percept_A %>%
  ...
  geom_vline(xintercept=0) +
  geom_vline(xintercept=max_RT,linetype = 3) +
  geom_rect(mapping=aes(xmin=0, xmax=120, ymin=2.84, ymax=2.9), color="black", alpha=0.5) +
  geom_segment(data = lines_a,
               aes(x= xstart, xend = xend,
                   y = ystart, yend = yend),
               linewidth = 1/3,
...
create categorization + SOA plot
...
```{r}
ggsave("manuscript/Suppl_material/Plot_paradigms.png",dpi = 800,width = 16,height = 17)
```
