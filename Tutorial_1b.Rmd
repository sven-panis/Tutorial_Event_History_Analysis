...
```{r read-raw-data}
raw <- read_delim(
  file = "Tutorial_1_descriptive_stats/data/DataExp1_6subjects.txt",
  delim = "\t",
  col_names = T
)
head(raw)
...
data_wr <- data_wr |>
  select(-c(
    ss, practice, rd, form, task, primeduration,
...
data_wr <- data_wr |>
  rename(
    pid = vp, condition1 = mask_type, condition2 = prime_type,
