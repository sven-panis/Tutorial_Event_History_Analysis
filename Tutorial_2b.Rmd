...
As we expect conditional accuracies ranging between (and including) 0 and 1, we use a flat prior on the logit scale for our intercepts (see section E in the supplemental material).
...
```{r epred-grand-plot}
 p1 <- ggplot(epreds_grand, aes(x=timebin, y=.epred, 
    ...
    labs(x = "time bin", y = "predicted conditional accuracy") #+
...
p11 <- ggplot(epreds_grand_diffs, aes(x=timebin, y=diff_ca, 
 ...
 # stat_halfeye(point_interval = "mean_qi",.width=c(.8,.95))+
  ...
  scale_color_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle=90)) +
  labs(x = "time bin", y = "difference in predicted ca") +
...
# save combined plot
ggsave("Tutorial_2_Bayesian/figures/M1i_ca_ame_combined.png", width = 10, height = 12, dpi = 800)
```
