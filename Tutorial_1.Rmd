...
Combine plots of 2 subjects.

```{r plot-two-subjects}
pa <- pluck(data_final, 8, 3)
pb <- pluck(data_final, 8, 6)

pa | pb 
```

```{r save-plot-two-subjects}
ggsave("Tutorial_1_descriptive_stats/figures/Plot_subjects3and6_PanisSchmidt.png", width = 8, height=8, dpi=600)
```
...
# function plot_eha_agg plots the hazard, survivor, and ca(t) functions for aggregated data.
...
p1 <- df %>% ggplot(aes(x=period, color=condition, group=condition)) +
  ...
  scale_x_continuous(breaks = c(0,1:(cutoff/binsize)), labels=c(0,1:(cutoff/binsize)*binsize),
                     limits = c(0,cutoff/binsize)) +
  scale_y_continuous(limits = c(0,haz_yaxis)) +
  labs(x="", y="h(t)", title = "Aggregated data (N=6)") +           # New info in title
  theme(legend.background = element_rect(fill = "transparent"),
        panel.grid = element_blank(),
        legend.position = "top")
# plot the survivor functions
