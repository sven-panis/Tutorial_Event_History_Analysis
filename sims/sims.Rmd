...
This file simulates the data used in old Figures 1, 2, and 3 of the manuscript and then creates some plots.

Figure 1 of revision 1 can be found in section 13.
...
p13.1 <-ggplot(lifetable_fig1, aes(x = bin, y = hazard,
  ...
  geom_linerange(aes(ymin=hazard-se_haz, ymax=hazard+se_haz), show.legend = F) +
  scale_colour_brewer(palette = "Dark2") + 
  scale_x_continuous(breaks = 1:10, limits = c(1,10),labels = c(1:10)*100) +
  scale_y_continuous(limits = c(0,1)) +
 # ggtitle("hazard and conditional accuracy functions") +
  xlab("time bin t's endpoint (ms)") +
  ylab("hazard") +
