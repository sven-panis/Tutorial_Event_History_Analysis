...
# Section 11: Simulate hazard data for 1 subject, 1 condition, 200 trials, and plot empirical hazard and survivor functions with error bars (Figure 2)
...
## apply function sim_haz
data_single2 <- sim_haz(subj_n = 1, rep_n = 1000) %>%
               mutate(period = bin,
               ...
               mutate(acc = ifelse(period<6 & condition=="incongruent",0,1),
                      acc = ifelse(period==6 & condition=="incongruent" & trial < 500,0,acc)) 
...
acc <- tibble(
  time = seq(1,10,1),
  congruent = rep(1,length = 10),
  incongruent = c(0, 0, 0, 0, 0, .5, 1, 1, 1, 1)
) %>% 
  pivot_longer(-time,
               names_to = "condition",
               values_to = "accuracy") %>% 
