...
## create a function that can simulate hazrad functions between conditions ##
...
use a truncated normal distribution to privude upper and lower bounds
...
```{r}
rt1_pid <- summary_pid(rt1) %>%
  mutate(example = "example 1")
  
rt1_group <- summary_group(rt1) %>% 
  mutate(example = "example 1")


rt2_pid <- summary_pid(rt2) %>% 
  mutate(example = "example 2")

rt2_group <- summary_group(rt2) %>% 
  mutate(example = "example 2")


rt3_pid <- summary_pid(rt3) %>% 
  mutate(example = "example 3")

rt3_group <- summary_group(rt3) %>% 
  mutate(example = "example 3")
```
...
```{r}
rt4_pid <- summary_pid(rt4) %>%
  mutate(example = "example 4")
  
rt4_group <- summary_group(rt4) %>% 
  mutate(example = "example 4")


rt5_pid <- summary_pid(rt5) %>% 
  mutate(example = "example 5")

rt5_group <- summary_group(rt5) %>% 
  mutate(example = "example 5")


rt6_pid <- summary_pid(rt6) %>% 
  mutate(example = "example 6")

rt6_group <- summary_group(rt6) %>% 
  mutate(example = "example 6")
```
