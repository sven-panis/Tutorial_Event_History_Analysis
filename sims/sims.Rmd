...
This file simulates the data used in Figures 1, 2, and 3 of the first version of the manuscript and then creates some plots.

Figure 1 of revised manuscript can be found in section 13.
...
# Section 11 (Figure 2): Simulate hazard data for 1 subject, 1 condition, 200 trials, and plot empirical hazard and survivor functions with error bars

## Simulate hazard data based on user-defined hazard function
...
data_single <- sim_haz(subj_n = 1, rep_n = 200) %>%
               filter(condition == "cond1") %>%
               mutate(period = bin,
                      trial = str_sub(rep, start=6, end=6)) %>%
               select(-c("contrast","cumsum","outcomel","outcomen","bin","rep")) 
head(data_single, n=20)
```

## set up life table

```{r}
lifetable <- data_single %>%
...
## wrangle life table for plotting

```{r}
minimallt <- lifetable %>% select(period,condition, hazard, se_haz, survival, se_surv) %>%
  pivot_longer(cols = c(hazard,survival),
               names_to = "dv",
               values_to = "probability") %>%
  mutate(se = ifelse(dv=="hazard", se_haz, se_surv))
```

## plot hazard and survivor functions

```{r}
p11.1 <-ggplot(minimallt, aes(x = period, y = probability,
                         colour = dv)) +
  geom_line(linewidth = 1) +
  geom_point(size=3) +
  geom_linerange(aes(ymin=probability-se, ymax=probability+se), show.legend = F) +
  scale_colour_brewer(palette = "Dark2") + 
  scale_x_continuous(breaks = 1:10) +
  scale_y_continuous(limits = c(0,1)) +
  ggtitle("hazard and survivor functions") +
...
ggsave("sims/figures/haz_surv_single.jpeg",
       width = 7, height = 5, dpi=800)
```


# Section 12 (Figure 3): plot of hazard and ca(t) for 2 conditions with error bars
...
data_single2 <- sim_haz(subj_n = 1, rep_n = ntrials) %>%
               mutate(period = bin,
                      trial = as.integer(str_sub(rep, start=4, end=7)),
                      condition = factor(condition, labels=c("congruent","incongruent"))) %>%
               select(-c("contrast","cumsum","outcomel","outcomen","bin","rep"))  %>%
               mutate(acc = ifelse(period<6 & condition=="incongruent",0,1),
                      acc = ifelse(period==6 & condition=="incongruent" & trial < ntrials/2,0,acc)) 
head(data_single2, n=20)
summary(data_single2)
```

## set up life table

```{r}
lifetable2 <- data_single2 %>%
    mutate(event = str_c("event", event)) %>%
    group_by(condition,period) %>% 
    count(event) %>% 
    ungroup() %>% 
...
```{r}
p12.1 <-ggplot(lifetable2, aes(x = period, y = hazard,
                         colour = condition)) +
  geom_line(linewidth = 1) +
  geom_point(size=3) +
  ...
  scale_y_continuous(limits = c(0,1)) +
  ggtitle("hazard")  +
  xlab("time bin") + ylab("probability") + guides(color=guide_legend(title="prime type"))
p12.1
```
...
```{r}
p12.2 <-ggplot(ca, aes(x = period, y = ca,
                         colour = condition)) +
  geom_line(linewidth = 1) +
  geom_point(size=3) +
  ...
  guides(color=guide_legend(title="prime type"))
p12.2
```
...
#ggsave("sims/figures/haz_acc_single.jpeg",
#       width = 7, height = 6, dpi=800)
...
# section 13: REVISION 1: Create New Figure 1
...
```{r}
p13.1 <-ggplot(lifetable_fig1, aes(x = bin, y = hazard,
                         colour = condition)) +
 ...
 # annotate("label", x = 1.7, y = .95,
#           label = "h(t) = P(RT = t | RT ≥ t)",
...
```{r}
p13.2 <-ggplot(ca, aes(x = bin, y = ca,
                         colour = condition)) +
  geom_line(aes(linetype=condition),
            linewidth = 1) +
  geom_point(size=3) +
 ...
 #  annotate("label", x = 1.5, y = .25,
#         label = "ca(t) = P(correct | RT = t)",
#           fill = "white",           # White background
#           color = "black", 
#           size = 7, 
#           hjust = 0) +
 ...
 # ggtitle("conditional accuracy functions") 
#+ 
#  guides(color=guide_legend(title="prime type"))
p13.2
```
...
# Create right column (p13.1 and p13.2) with collected axes  
...
```{r}
ggsave("sims/figures/Fig1_revision1.jpeg",
...
# section 14: REVISION 2: Create Supplementary Figure to illustrate the effect of bin width
...
```{r}
ggsave("sims/figures/Fig_revision2_binwidth.jpeg",
